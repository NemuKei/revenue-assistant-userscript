...
- context の追加・更新・再検証・失効・退避は、利用者が lifecycle 判断を明示した場合だけ `context-lifecycle` へ route する。通常の closeout や履歴保存を自動発火させない。
...
- フロントエンド作業は共有 `frontend-skill` を capability router とし、利用者の明示指定を優先したうえで、現在利用可能な design / engineering owner を1つ選ぶ。Product Design は固定依存にしない。
