...
- `旧版の技術負債や内部構造を引き継ぐこと` と `旧版で実務利用されてきた Analyze の見方や操作を維持すること` は分けて判断する。内部は clean-room で作り直す一方、booking curve、参考線、過去調整 marker、競合価格 graph、filter、toggle などの慣れた見方と操作は設計の出発点としてできるだけ踏襲する。標準 UI との干渉、意味の誤り、または実利用で判断が遅くなる根拠がある場合だけ、置き換える部分と理由を明示して再設計する。
