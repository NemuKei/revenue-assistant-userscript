...
- `API 負荷を抑える初期の取得上限を固定すること` と `実際の操作待ち時間を実務で使える範囲にすること` が競合する場合は、まず実画面で所要時間と失敗を確認する。速度が実務の妨げになる場合は、取得対象、停止条件、重複回避、認証・rate limit時の停止を維持したまま、開始間隔、同時数、1回上限の段階的な緩和を検討する。実測前に無制限化しない。
