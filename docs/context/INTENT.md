...
- `Topのbackground coverageを早く完了すること` と `Analyzeで全体、選択room current、選択room根拠をテンポよく確認できること` が競合する場合は、Analyzeを優先する。Topの表示中cueに必要なforegroundは維持するが、画面外の補完や学習用取得はAnalyzeの判断可能時間を使い切らない範囲へ縮退させる。総request上限を増やす前に、既存上限内でbackground admissionとforeground reserveを再配分する。
