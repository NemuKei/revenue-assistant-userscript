...
この節の候補は、2026-06-04 の後続 task 化で `docs/tasks_backlog.md` の未着手 task 本体と Remaining Task Triage へ反映済みである。
実行順の正本は `docs/tasks_backlog.md` の `Remaining Task Triage` とする。

追加で、利用者が示した次の 2 件も `docs/tasks_backlog.md` へ追加した。

- `RAU-UX-116`: 右下 warm cache indicator を廃止し、必要な状態表示を画面内の文脈へ移す。
  - 目的: 右下固定表示が視覚的に邪魔になっているため、固定 indicator をなくし、取得状態は top 画面、Analyze 画面、価格推移 / 競合価格 tab の該当領域へ移す。
  - 実行順: Now。
- `RAU-UX-117`: `候補データ優先取得` strip の初期表示位置を料金調整候補側に固定する。
  - 目的: strip が最初にカレンダー上部へ表示され、その後に料金調整候補 list 側へ移動する挙動をなくす。
  - 実行順: Next。
