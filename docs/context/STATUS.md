...
1. 月次実績画面の DOM と API を調査し、カスタム booking curve の最小仕様とデータ取得可否を切り分ける
2. `同月同曜日` baseline の最小表示仕様と、それに伴う `IndexedDB` 導入要否を判断する
3. booking curve の標準 UI に `団体` 系列を含めるかを、実装後の使用感ベースで再判断する
