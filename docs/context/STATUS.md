...
1. `docs/tasks_backlog.md` の Remaining Task Triage を確認する。2026-05-31 時点で Now / Next / After Next / Later は空であり、未着手 task はない。
2. 次に進める場合は、今回の完了後に見えた追加観点を新規 task として切るか、公開版が更新された後の Tampermonkey dashboard 更新と配布版 smoke を独立した確認 task として扱うかを判断する。
...
- 2026-05-31 の `RAU-UX-48` から `RAU-UX-58` 完了時の verify:
  ...
  - GitHub Actions `Publish Userscript`: first run `26714118104` は `npm ci` lockfile mismatch で failed。lockfile 修正後の run `26714209781` は success。
  - `npm run userscript:version-check`: passed。local version `0.1.0`、published version `0.1.0.354`、通常 Chrome の Revenue Assistant root で login form candidate `no`、calendar candidate `yes`、RAU userscript root count `3`、React marker mounted `yes` を確認した。
  - Tampermonkey dashboard 更新: `Revenue Assistant Userscript` を `0.1.0.354`、590 KB に更新した。
  - `npm run smoke:distribution -- --installed-version 0.1.0.354 --mode top --seconds 30 --version-policy fail`: passed。top row 10 件、React marker mounted `yes`、target month select `yes`、view mode buttons 4 件、display limit buttons 1 件、rank order control `yes`、curve preview buttons 10 件、rank change buttons 10 件、decision buttons 20 件、console / page error 0 件、監視対象 write API POST 0 件だった。15 秒版は更新直後の初回 reload で RAU root 0 件の expected fail になったため、Tampermonkey 更新直後の top smoke は 30 秒以上待つほうが安定する。
