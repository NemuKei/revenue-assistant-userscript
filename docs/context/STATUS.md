...
- 次スレッドの範囲:
  ...
  - `RAU-CP-13` は実装済み、Chrome DevTools Protocol で GUI 確認済み。公式価格推移 data は既存 `competitor-price-snapshots` とは別に、IndexedDB database `revenue-assistant-price-trends` / store `price-trend-records` へ保存する。record は `facilityId`、`stayDate`、`numGuests`、`mealType`、`roomType`、`roomTypeLabel`、`fetchedAt`、`endpoint`、`query`、`facilities`、`scope`、`payload` を持つ。`roomType` と `roomTypeLabel` は、公式 API で部屋タイプ条件が確認できない場合は `null` とする。料金調整候補 scoring への接続はまだ行わない。最新 `dist` の一時注入確認では、`価格推移` タブで RAU overview 1 件、panel 4 件、SVG 4 件、別 store record 4 件、人数 1 から 4、各 record の `payload.yads` 6 件を確認した。追加確認では、`価格推移` タブの RAU overview 1 件、既存競合価格 overview 0 件、見出し `競合価格 最安値推移（90日版）`、filter label `部屋タイプ` / `食事`、panel title `1名 最安値` から `4名 最安値`、tooltip header `部屋タイプ` / `価格` / `前回差分` / `自社との差`、tooltip row 6 件、first cell `自社 / 指定なし`、page error 0 件、console error 0 件を確認した。`競合価格` タブへ切り替えると価格推移 overview は 0 件、競合価格 overview は 1 件で、競合価格側 tooltip header も同じ 4 列だった。89日超過候補の `2026-09-01` と `2026-09-30` は公式側の `price-trends-content` が表示されず、RAU の対象外表示は今回の GUI では確認できなかった。
