...
- 主対象: 2026-06-04 に、利用者が `@product-design` を使って既存機能画面を順次ブラッシュアップする task 作成を依頼したため、個別画面をいきなり改修せず、全画面を Product Design audit 対象として棚卸しする `RAU-UX-106` から `RAU-UX-111` を `docs/tasks_backlog.md` に追加した。`RAU-UX-106` は Product Design `get-context` brief、対象 surface、capture 手段、desktop / mobile viewport、評価軸を固定する docs-first task である。`RAU-UX-107` は top 画面を audit し、直近でブラッシュアップ済みの候補 list と月別優先取得 strip を全画面 audit の基準点として確認する。`RAU-UX-108` は Analyze 販売設定画面、`RAU-UX-109` は Analyze 競合価格 / 価格推移 tab を audit し、スクリーンショット証跡、UX findings、アクセシビリティ risk、次に実装する task ID を完了条件にする。`RAU-UX-110` は `INTENT.md` の優先順位に従い、月次実績画面を Later task として扱う。`RAU-UX-111` は各画面 audit で共通して出た button、badge、filter、tooltip、loading、empty、mobile layout、fixture、smoke の改善を、依存追加や大規模 design system 化なしで切り出す task である。Remaining Task Triage は Now `RAU-UX-106`、Next `RAU-UX-107`、After Next `RAU-UX-108`、`RAU-UX-109`、Later `RAU-UX-110`、`RAU-UX-111` とした。今回の task 作成では、runtime UI、`src/`、`dist/`、Tampermonkey installed version、Revenue Assistant API request 範囲、Revenue Assistant write API は変更していない。docs verify は `git diff --check -- docs/tasks_backlog.md docs/context/STATUS.md` が通過済みである。
