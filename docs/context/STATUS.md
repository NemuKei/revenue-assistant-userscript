...
- 主対象: 2026-06-02 に、月別優先取得後の利用者導線と配布版 smoke に関する推奨 3 件を `RAU-WC-24`、`RAU-WC-25`、`RAU-WC-26` として task 化した。`RAU-WC-24` は、月別優先取得の完了後に対象月の料金調整候補が増えたか、判断が変わったか、根拠不足が減ったかを短く表示する summary を設計する docs-first task である。`RAU-WC-25` は、配布版 smoke で warm cache の HTTP status count、平均 request 開始件数、最小 request 開始間隔、worker 数、監視対象 write API POST 0 件を raw body なしで自動判定する task である。`RAU-WC-26` は、月別優先取得 button の配置と、button 押下後に候補 list の `対象月` filter またはカレンダー表示月を連動させるかを設計する docs-first task である。live backlog の重複確認では `RAU-WC-24`、`RAU-WC-25`、`RAU-WC-26` は存在せず、選択された 3 件と同じ責務境界の未着手 task も見つからなかった。Remaining Task Triage は Now `RAU-WC-26`、Next `RAU-WC-24`、After Next `RAU-WC-25`、Later なしとした。`RAU-WC-26` を最初に置く理由は、優先取得 button の配置見直しと候補 list 側の操作導線を先に決めないと、`RAU-WC-24` の summary 表示場所を決めにくいためである。今回の task 化では、runtime UI、`src/`、`dist/`、Revenue Assistant API request 範囲、Revenue Assistant write API、Tampermonkey installed version、Revenue Assistant 実画面状態は変更していない。
