...
- 次スレッドの範囲:
  ...
  - `RAU-CP-11` は read-only 調査済み。公式 Analyze `価格推移` タブで `tab-priceTrends`、`price-trends-content`、filter、chart header、updated-at の DOM を確認した。短時間観測では専用 `価格推移` endpoint は確定していないため、公式 data を RAU の snapshot store へ混ぜない。HAR、raw trace、request body、response body、Cookie、token、credential、非公開価格データは repo に保存していない。
  - `RAU-CP-12` は実装済み、Chrome DevTools Protocol で GUI 確認済み。公式 `価格推移` タブの `price-trends-content` にも、既存 `競合価格` タブと同じ保存済み snapshot 由来の `競合価格 最安値推移` graph を表示する。既存 `競合価格` タブの graph と IndexedDB snapshot store は削除、置換しない。GUI 確認では `価格推移` タブと `競合価格` タブの両方で overview 1 件、panel 4 件、SVG 4 件、empty 表示 0 件、page error 0 件、console error 0 件だった。
