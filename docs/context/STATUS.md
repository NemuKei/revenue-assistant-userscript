...
- analyze 画面で debug フラグを有効化した上で月送りや販売設定表示を実操作し、`queueCalendarSync()` の debug summary から残る支配的な発火源を確認する

## Next

1. analyze 画面で `localStorage["revenue-assistant:debug:calendar-sync"] = "1"` を有効化し、月送りや販売設定表示で `queueCalendarSync()` debug summary の支配的な reason を確認する
...
- 現在地は Phase 2 の最初の性能改善として、販売設定カードが見えていない状態では sales-setting 向け booking_curve prefetch を止め、booking_curve 比較値の事前集計共有、`queueCalendarSync()` の署名ベース重複抑止、reason 付き debug summary、通常ビルド向け debug フラグまで反映済み
- 直近の保存点は `2c35a9b` `Close booking curve phase 1` と `eb45646` `Skip hidden sales-setting prefetch`
- 次スレッドの最初の実装対象は、analyze 画面で `localStorage["revenue-assistant:debug:calendar-sync"] = "1"` を有効化し、月送りと販売設定表示を実操作して console の `calendar sync debug` summary から支配的な reason を確定するところから始める
- 先に保持すべき公開挙動は、Phase 1 の booking curve UI、tooltip close、`ACT` 空表示、rank marker overlay を変えないこと
- 次の最小差分候補は、debug summary で支配的だった reason だけを対象に、interaction timeout 群か consistency check のどちらが支配的かを切り分けること
