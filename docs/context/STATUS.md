...
- 主対象: `RAU-RR-08` rank change history による resolved 化を実装する
- 完了済み前提:
...
- 次スレッドの範囲:
  ...
  - 次の主対象は `RAU-RR-08` とする。`/api/v3/lincoln/suggest/status` の rank change history を使い、recommendation 生成後または asOfDate 以降に rank 変更が確認された候補を active list から外す resolved 化を実装する。
...
- 終了条件:
  - `RAU-RR-08` で、rank change history による resolved 化が first phase の候補 lifecycle として動く。
...
1. `docs/tasks_backlog.md` の `Now` にある `RAU-RR-08` を確認し、トップ表示範囲の `/api/v3/lincoln/suggest/status` を読み、roomGroup ごとの rank change history を候補 lifecycle に接続する。
2. recommendation の `asOfDate` または `generatedAt` 以降に同じ `stayDate x roomGroup` の rank change がある場合、candidate を active list から外す。
3. `RAU-RR-09` から `RAU-RR-11` は、first phase 実装後に rank response、推奨 rank 算出、bulk apply feasibility の確認済み範囲を正本化する。bulk apply は実装しない。
