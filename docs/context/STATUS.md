...
- 自前 DOM mutation 除外後の analyze 画面で、月送りや販売設定表示を再実測し、`queueCalendarSync()` debug summary の `mutation-observer` 件数がどこまで減るかを確認する

## Next

1. analyze 画面で `localStorage["revenue-assistant:debug:calendar-sync"] = "1"` を有効化し、月送りや販売設定表示で `queueCalendarSync()` debug summary の `mutation-observer` 件数が減ったかを確認する
...
- 現在地は Phase 2 の最初の性能改善として、販売設定カードが見えていない状態では sales-setting 向け booking_curve prefetch を止め、booking_curve 比較値の事前集計共有、`queueCalendarSync()` の署名ベース重複抑止、reason 付き debug summary、通常ビルド向け debug フラグ、自前 DOM mutation 除外まで反映済み
- 直近の保存点は `2c35a9b` `Close booking curve phase 1` と `eb45646` `Skip hidden sales-setting prefetch`
- 次スレッドの最初の実装対象は、analyze 画面で `localStorage["revenue-assistant:debug:calendar-sync"] = "1"` を有効化し、月送りと販売設定表示を再実操作して console の `calendar sync debug` summary から `mutation-observer` がどこまで減ったかを確認するところから始める
...
- 自前 DOM mutation 除外後の `queueCalendarSync()` 実行回数を、月送りや販売設定表示で再実測する
