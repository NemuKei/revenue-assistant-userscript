...
1. `RAU-AF-04` として、Revenue Assistant の `/api/v4/booking_curve` response 群を `stay_date x LT` の rooms matrix へ変換する純粋関数の置き場所を決める。
2. BCL repo の `recent90w` 相当と seasonal component 相当を、RAU の LT tick と `all / transient` 系列で扱える TypeScript 関数として実装する。
3. 実装中に final rooms の解決規則または履歴不足時の扱いが仕様本文とずれた場合は、先に `docs/spec_001_analyze_expansion.md` を更新する。
4. `RAU-AF-04` の verify が通ったら、次は `RAU-AF-05` の IndexedDB derived cache と request scheduler へ進む。
