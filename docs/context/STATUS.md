...
- 次スレッドの範囲:
  ...
  - 次の 1 task はまだ task 化しない。次に task 化する候補は、実利用で `その他` details の開閉が多い場合の操作再配置、status badge の `確認不足` と `送信不可` の優先順位再評価、Chrome DevTools Protocol smoke helper の websocket connected 後 timeout の原因切り分け、React Doctor の残り 49 issues から次に安全な 1 family を選ぶことである。
...
1. `docs/tasks_backlog.md` の Remaining Task Triage を確認する。2026-06-01 時点では Now、Next、After Next、Later の未着手 task はない。
2. 次にタスク化する場合は、`RAU-UX-83` から `RAU-UX-86` の完了結果を前提に、`その他` details の開閉頻度、status badge の優先度と文言、`smoke:distribution` の Chrome DevTools Protocol timeout、残り React Doctor issue family のどれを次の作業単位にするか決める。
