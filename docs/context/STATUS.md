...
- 主対象: 2026-06-02 に、未着手だった `RAU-WC-18`、`RAU-WC-19`、`RAU-WC-21`、`RAU-WC-22` を完了し、`RAU-WC-20` と `RAU-WC-23` は配布版実ブラウザ確認待ちに進めた。`RAU-WC-18` では、`/api/v4/booking_curve` の RAU 発行 read-only request を、無制限化ではなく `開始間隔 350ms 以上`、`同時実行 3 件以下` の上限付き高速化として `docs/spec_001_analyze_expansion.md` と D-20260602-008 に記録した。HTTP 401 はログイン確認、HTTP 403 は権限確認、HTTP 429 は cooldown、HTTP 5xx と network error は最大 2 回 retry とした。`RAU-WC-19` では、`bookingCurveRequestScheduler` を `350ms / concurrency 3` にし、warm cache queue を最大 3 worker で処理するよう変更した。`RAU-WC-21` と `RAU-WC-22` では、トップカレンダー上部へ表示月ごとの `YYYY-MM 優先取得` ボタン、円形 progress、`未優先`、`待機中`、`取得中 n%`、`完了`、`クールダウン中`、`エラー n` の状態表示を追加した。押した月の全 stay_date は通常の直近日付 queue より前に置く。Revenue Assistant write API、rank 変更 POST、自動反映、一括反映、価格や在庫の非公開データ保存、IndexedDB schema、raw source compact schema、candidate scoring、priority、confidence、reasonFingerprint は変更していない。ローカル verify は `npm run typecheck`、`npm run lint`、`npm run build`、`npm run build:vite:fixture`、`npm run check:fixture-markers` が通過済みである。Vite と esbuild を起動する build / fixture / marker は sandbox 内で `spawn EPERM` になったため、同じ command を昇格して実行した。Remaining Task Triage は Now `RAU-WC-20`、`RAU-WC-23`、Next / After Next / Later なしであり、次は commit / push 後に GitHub Pages published version と Tampermonkey installed version を揃え、配布版 top smoke と CDP 観測で HTTP error、console error、page error、監視対象 write API POST 0 件を確認する。
