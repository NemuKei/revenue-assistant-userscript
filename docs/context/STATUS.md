...
- 次スレッドの範囲:
  ...
  - `RAU-CP-12` は方針を更新し、公式 `価格推移` タブでは既存 `competitor-price-snapshots` 由来 graph を使わない。`price-trends-content` には `/api/v1/price_trends` から取得した人数 1 から 4 の lead time 別 graph を表示する。表示 series は `自社`、`競合最低価格`、競合施設別 series とし、既存 `競合価格` タブの snapshot graph と IndexedDB snapshot store は削除、置換しない。見出しは `競合価格 最安値推移（90日版）` とし、価格推移側にも `部屋タイプ` と `食事` の絞り込み UI を表示する。tooltip は `競合価格` タブと `価格推移` タブの両方で `部屋タイプ`、`価格`、`前回差分`、`自社との差` に揃える。部屋タイプ選択時は、指定なし response の後処理ではなく、対応する `room_type_options[]` 単独指定で保存した record を使う。
  - `RAU-CP-13` は実装済み、Chrome DevTools Protocol で GUI 確認済み。公式価格推移 data は既存 `competitor-price-snapshots` とは別に、IndexedDB database `revenue-assistant-price-trends` / store `price-trend-records` へ保存する。record は `facilityId`、`stayDate`、`numGuests`、`mealType`、`roomType`、`roomTypeLabel`、`fetchedAt`、`endpoint`、`query`、`facilities`、`scope`、`payload` を持つ。`roomType` と `roomTypeLabel` は、指定なし request では `null`、部屋タイプ別 request では `SINGLE`、`DOUBLE`、`TWIN`、`TRIPLE`、`FOUR_BEDS`、`WASHITSU`、`WAYOUSHITSU` のいずれかと表示 label にする。料金調整候補 scoring への接続はまだ行わない。最新 `dist` の一時注入確認では、`価格推移` タブで RAU overview 1 件、panel 4 件、SVG 4 件、別 store record 4 件、人数 1 から 4、各 record の `payload.yads` 6 件を確認した。追加確認では、`価格推移` タブの RAU overview 1 件、既存競合価格 overview 0 件、見出し `競合価格 最安値推移（90日版）`、filter label `部屋タイプ` / `食事`、panel title `1名 最安値` から `4名 最安値`、tooltip header `部屋タイプ` / `価格` / `前回差分` / `自社との差`、tooltip row 6 件、first cell `自社 / 指定なし`、page error 0 件、console error 0 件を確認した。部屋タイプ別取得確認では、同一 fetch run の 32 record が保存され、scope は `指定なし`、`SINGLE`、`DOUBLE`、`TWIN`、`TRIPLE`、`FOUR_BEDS`、`WASHITSU`、`WAYOUSHITSU` と人数 1 から 4 の組み合わせだった。UI では `ツイン` 選択後の tooltip first cell が `自社 / ツイン` になった。`競合価格` タブへ切り替えると価格推移 overview は 0 件、競合価格 overview は 1 件で、競合価格側 tooltip header も同じ 4 列だった。89日超過候補の `2026-09-01` と `2026-09-30` は公式側の `price-trends-content` が表示されず、RAU の対象外表示は今回の GUI では確認できなかった。
