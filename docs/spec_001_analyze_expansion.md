...
- 1 回の自動 warm cache 稼働時間に達した場合は、日次上限に達していなければクールダウン時間を置いた後に自動再開する。
- 日次上限に達した場合は同日中の自動再開を行わず、indicator で日次上限到達として表示する。
...
- 最小表示は `待機中`、`取得中 完了日数 / 対象日数`、`一時停止中`、`クールダウン中`、`上限到達`、`エラー n` を区別する。
