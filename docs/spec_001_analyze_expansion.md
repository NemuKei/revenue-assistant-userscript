...
5. `RAU-PERF-22`では、Top background counterとAnalyze foregroundを分離し、Analyzeは固定request-count ceilingを持たない。販売設定のroom currentを並列queue / progressive描画へ変える。`D-20260813-007`の目的、対象endpoint、before / after、storage unchanged、負荷guard、停止、rollback、利用者承認をYellow zone contractとし、実装後はcold / delta双方をlive markerで確認する。
