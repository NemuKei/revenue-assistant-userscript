...
- データ取得速度の確認用に、RAU は `data-ra-fetch-performance-summary` の JSON marker を DOM へ出してよい。marker は price_trends の表示要求時刻、保存済み record 初回描画時刻、visible / background fetch の開始・完了時刻、visible / background scope count、cache read count、network fetch count、error count と、booking_curve warm cache の queue build 時刻、表示候補 currentRaw の fetched / skipped / errored count、preScanHitCount だけを持つ。`localStorage["revenue-assistant:debug:fetch-performance"] === "1"` の場合だけ同じ sanitized summary を console に出してよい。response body、価格詳細、施設実データ、予約・在庫・顧客情報、Cookie、token、credential は marker、console、storage、docs に保存しない。この marker は計測補助であり、価格推移 API query、`PriceTrendRecord` 保存 schema、booking_curve raw source 保存 schema、request 順序、scheduler、cache freshness、network skip、Revenue Assistant write API は変更しない。

Candidate Performance Contracts (未実装候補):

- この節は `RAU-PERF-14` から `RAU-PERF-18` と `RAU-UX-134` のための planned performance improvements であり、現時点では未実装の計画である。既存の runtime code、`dist/`、Revenue Assistant API request、保存 schema、API query contract、request 総数、Revenue Assistant write API、rank change payload は変更しない。
- `RAU-PERF-14` では、`RAU-PERF-09` の live observation 未完了 / blocked 履歴を引き継ぎ、price_trends、booking_curve、competitor 関連の baseline 再取得項目を定義する。live observation が取れる場合は CDP 9222 または通常 Chrome / Tampermonkey 実行版で `data-ra-fetch-performance-summary` を読む。取れない場合は fixture + manual observation の fallback 手順を使い、duration、request count、error count、timestamp、marker 有無だけを比較対象にする。
- `RAU-PERF-15` では、competitor visible / user-facing fetch だけを bounded parallel fetch の候補にする。初期候補は最大 2 並列であり、background / cold queue は現行の抑制を維持する。request 総数を増やす前提ではなく、競合価格 snapshot の検索条件 signature、保存 schema、API query contract は変えない。
- `RAU-PERF-16` では、保存済み competitor price snapshot がある場合に network 完了前の cache-first 表示を候補にする。保存済み表示時は `保存済み表示・再取得中` 相当の meta を出し、freshness 未確定のまま network fetch を skip しない。最新取得完了後は通常表示へ更新し、stale cache を最新値として誤認させない。
