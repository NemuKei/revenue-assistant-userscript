...
Analyzeの判断テンポはTopの画面外background coverage完了より優先する。次のforeground保護sliceでは、Analyzeのoverall current、選択room current、選択room reference / rank markerをTop backgroundより先にadmitし、critical / selected taskがpendingの間は新しいbackground requestを開始しない。開始済みrequestはpriority elevationだけを理由にabortしない。Top background plannerは、将来同じsessionでAnalyzeへ進んだ際のbounded headroomを使い切ってはならない。最初はbootstrap 800 / daily 200の総上限内でbackground上限とforeground reserveを再配分し、reserve値はroom数bandとcurrent / selected-referenceのpure task count、通常Chrome markerから実装前に固定する。総上限、scope、long-session tokenを増やす変更はこのsliceに含めない。
...
5. `RAU-PERF-22`では、利用者が明示したAnalyze優先と、cold Top 768 + Analyze 32後に選択room根拠がsettleしなかったlive evidenceを受け、既存総上限内のinteractive admission、background pause、bounded foreground reserveを先に設計する。総request上限、scope、standard画面より多い取得を変える場合だけ、同じshared-budget原因の追加再現、before / after、追加request上限、停止条件、rollbackをYellow zone decisionへ固定してから実装する。
