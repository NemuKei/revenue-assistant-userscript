...
10. 公開Next `0.2.0.33`では翌日宿泊日のoverall 790ms、6 room summary 3,332ms、room current 16ms、selected evidence 2,478msまで短縮したが、reference 62件の連続保存中にfull scopeを反復hydrateし、Long Task最大1,006ms、軽量probe最大遅延1,058msを観測した。`RAU-PERF-25`では通信profileを維持し、currentがsettle済みのfull reference batchだけをtrailing debounceへ変え、静止後の1回でfresh evidenceを反映する。公開Next `0.2.0.34`ではoverall / 6 room summary各20ms、room current 22ms、selected evidence 1,535ms、reference planned / started 62 / 62、最大同時20、scheduler error / stop 0だった。一方、120回の軽量marker readは最大833ms、500ms超8回で、今回のBrowser経路では`PerformanceLongTaskTiming`件数とnetwork / write streamを取得できなかった。jank解消を断定せず、`RAU-PERF-26`でmain-thread stallのphaseを分けるまで同時実行数を20から増やさない。
11. `RAU-PERF-26`では公開Next `0.2.0.34`に残った500〜833msの操作応答遅延を、request密度を変えずにphase別へ分ける。performance marker schemaを`rau-next-performance-v2`へ上げ、`responseRead`、`responseParse`、`responseCompact`、`sourceRead`、`sourceBuild`、`sourceWrite`、`storedNotify`、`acquisitionPublish`、`referenceRead`、`curveBuild`、`curveRender`、`markerPublish`だけを固定phaseとして、operation中のcount / total ms / max msをpage memoryへ集計する。`responseRead`、`sourceRead`、`sourceWrite`、`referenceRead`は非同期I/Oのwall elapsedであり、並行区間のtotalは重複し得るためmain-thread CPU時間へ読み替えず、同期phaseのmax / totalとLong Taskを合わせて原因を判断する。Long Taskはobserverの`active / unsupported / error`、count / total ms / max msだけを同じrunへ持ち、context開始前のentryを混ぜない。phase計測はrequestごとのmarker DOM書換えを増やさず、終端milestoneまたはcoalesced flushで反映する。施設、stay date、room、価格・在庫、request / response body、storage key、raw entry、URLは保持しない。公開Next `0.2.0.36`のwarm liveではbooking acquisition 0件でも初期`curveBuild` 4回 / 3,700ms、最大1,579msとLong Task最大1,599msが一致し、room openでも`curveBuild` 756msとLong Task最大814msが一致したため、通信ではなくcurve model構築を主因とする。
12. `RAU-PERF-27`ではrequest profileを維持し、初期hotel curveをroom batch完了時に再構築せず、rank loading / settledでcurve modelを更新する対象を実際に開いているroomへ限定する。reference inputは画面に表示する`全体 + 選択中の個人または団体`の2 segmentだけをmaterializeし、直近型はobservationをLTで1回index化して各tickへ再利用する。current / recent / seasonal、rank marker、source count、欠損理由、scope別差分描画は維持する。foreground 35ms / concurrency 20、Top 50ms / 20、task集合、endpoint、保存schema / retention、retry / stop、Revenue Assistant write 0は変更せず、公開liveでLong Taskと`curveBuild`低下を確認するまでconcurrencyを増やさない。
13. 表示範囲外の半年取得`RAU-WC-34`は、SLO baselineとforeground保護を先に通し、余力だけを使う別scheduler / cadenceとして扱う。半年coverageの完了をTop / Analyze描画のbarrierにしない。
...
- booking curveの横軸は全LTのpoint / hitbox / Tooltip / accessible tableを維持したまま、表示labelだけを非重複の判断節目へ絞る。広幅は`360 / 270 / 180 / 90 / 60 / 45 / 30 / 21 / 14 / 7 / 3 / 0 / ACT`、狭幅は`360 / 180 / 90 / 30 / 7 / ACT`を表示し、`0日前`と`ACT`は互いに外側へ開く配置とする。labelを省いたLTもTooltipと表から確認できること。
- booking curveのhover / keyboard focusでは、選択LTを示すvertical guideと現在値のactive pointを表示する。currentが欠損するLTではguideは表示し、active pointだけを隠す。Tooltipはchart中央へ固定せず、mouseではcursor、keyboardでは選択LTの右へ間隔を空けて置く。chart panelの端は表示境界にせず隣のpanel側へ出してよく、browser viewportの端だけでclampする。rank markerのTooltipも同じ配置でmarker位置へ追従し、離脱時はTooltip、guide、active pointを同時に隠す。SVGの読み上げ名は`aria-label`、説明は固有`desc` + `aria-describedby`へ置き、browser native Tooltipを発生させるSVG `title`は持たない。
- 90日価格推移はClassicと同じく通常時は施設別lineを主役とし、全pointの常時markerは置かない。hover / keyboard focusでは選択lead time列を薄く着色し、vertical guideを表示し、Tooltipをcursorまたは選択lead timeの右へ間隔を空けて置く。panel端では押し戻さず、viewport端だけでclampする。SVGの読み上げ名は`aria-label`、説明は固有`desc` + `aria-describedby`へ置き、browser native Tooltipを発生させるSVG `title`は持たない。
- 競合価格履歴は既存のactive列、vertical guide、cursor / 日付追従Tooltip、施設別pointを維持する。TooltipはClassicと同じくcursorまたは選択日付の右へ間隔を空け、panel端を越えてよく、viewport端だけでclampする。SVGの読み上げ名は`aria-label`、説明は`aria-describedby`へ置き、browser native Tooltipを発生させるSVG `title`は持たない。マウス操作ではNext Tooltipだけを表示し、keyboard操作ではhitboxのfocus ringと同じNext Tooltipを維持する。
...
- 価格推移タブは、公式 `/api/v1/price_trends` から保存した `price-trend-records` を使う。初回表示では、表示中 stay_date の `roomType = 指定なし`、`mealType = NONE / BREAKFAST / DINNER / BREAKFAST_DINNER`、`guestCount = 1 / 2 / 3 / 4` の 16 request を優先する。これは既定の `部屋タイプ=指定なし`、`食事=指定なし`、人数別 4 panel を先に描画するための最小単位である。Nextは部屋タイプ別scopeをbackgroundで総当たり取得せず、利用者が部屋タイプfilterを明示選択した場合だけ、選択roomの不足する4食事 x 4人数を最大16 request、concurrency 2で取得する。保存済み record は `fetchedAt` を取得時刻として表示できるようにし、89 日より先または `yads` 空配列は対象外理由として扱う。
...
- 保存 scope は食事指定なし、1〜6名を共通条件とし、部屋タイプ指定なしと `SINGLE / DOUBLE / TWIN / TRIPLE / FOUR_BEDS` の単独指定を合わせた6 snapshotとする。部屋タイプ指定なしresponseは全タイプを網羅する契約ではないため、specific 5条件の代用にしない。一方、`SEMI_DOUBLE`など単独指定が確認済みでないタイプは新規requestへ追加せず、指定なしresponse内の実plan値として保持する。
- `facilityId x stayDate x JST取得日 x 部屋タイプscope` が同じ有効recordをClassicまたはNextの履歴に持つscopeは再取得せず、不足scopeだけを補う。6 scopeがすべて有効なら、その日は保存済みとして`/api/v2/competitors`と`/api/v5/competitor_prices`を発行しない。保存失敗は成功扱いにせず、次に標準競合価格本文を明示表示したときだけ不足scopeを再試行してよい。
- 未保存scopeの取得は、同一session、自施設、自分の権限内で既存の`GET /api/v2/competitors`を最大1回共有し、`GET /api/v5/competitor_prices`を不足scopeごとに最大6回、同時2件以下で使う。1回の表示で最大7 GETとし、response検証が全scopeで終わる前に新規recordを書かない。同一runの自動retry、週・月・前後日・他stay dateのbackground queue / prefetchは含めない。Revenue AssistantのPOST / PUT / PATCH / DELETE、自動再ログイン、認証回避、rate limit回避は行わない。
...
- Next 専用 store は同一`facilityId x stayDate`ごとに120観測日相当、最大720 recordを保持し、保存成功時に超過した古いNext recordだけを削除する。既存の部屋タイプ指定なしkeyは互換維持し、specific scopeだけroom typeを含むdeterministic keyを使う。Classic databaseとClassic recordは削除、移動、上書きしない。利用者が明示した削除UIやdatabase全削除は含めない。
...
- 競合価格履歴と90日価格推移のfilterは、共通の分類・表示契約を使う。部屋タイプは`シングル / セミダブル / ダブル / ツイン / トリプル / フォース / スイート / 和室 / 和洋室`の正規化と同じ順序、食事は`素泊まり / 朝食 / 夕食 / 朝夕食`を正とし、未選択はどちらも`指定なし`と表示する。2タブは同じbutton hierarchy、active state、keyboard focus、680px以下44px tap targetを使い、API固有のrequest valueと保存責務だけを各adapter / writerへ残す。
...
- 2026-07-23の利用者明示承認、2026-08-15の`D-20260815-001`に基づき、Nextは `/analyze/YYYY-MM-DD` の標準価格推移本文が実際に可視で、facility label guardが一致し、document visibleで、stay dateがJST当日から89日先までの場合だけ取得・保存を開始してよい。他tab、別route、document hidden、facility mismatch、90日範囲外では開始せず、開始後にcontextが外れた場合は未完了requestをabortする。
- 初期capture scopeは `roomType=null`、食事 `NONE` / `BREAKFAST` / `DINNER` / `BREAKFAST_DINNER`、人数1〜4の16scopeとする。利用者が部屋タイプを明示選択した場合だけ、そのroomの同じ16scopeを別batchとして取得する。Classic / Nextの有効recordを統合して同じJST取得日のscopeをskipし、選択batchの全16scopeが揃う場合はcompetitor / price trend requestを0件とする。不足時だけ `GET /api/v2/competitors`を最大1回、`GET /api/v1/price_trends`を不足scope数だけ最大16回、concurrency 2で使う。複数room自動取得、週・月・隣接日、他stay date、background prefetchは行わない。同じ表示contextでは成功、skip、unsupported、失敗をscope別にmemory再利用し、失敗を自動retryしない。
- price trend request queryは表示中stay date、scopeの人数 / 食事、自施設と現在の競合施設idを持ち、specific room batchだけ`room_type_options[]`へ選択roomを1件付ける。responseはroot、stay date、source更新時刻、yad id、lead time、税込価格、statusをruntime validationし、requested stay date / yad scopeと一致する有限値だけを正規化する。raw response、request / response body、HAR、Cookie、token、credential、予約・顧客情報、plan name、URLは保存しない。選択batchの全scopeが完了する前に失敗した場合はpartial batchを保存しない。
...
- 保存成功時だけ、Next store内で同一facility / stay date / guest / meal / roomの最新1件を残し、施設単位でstay dateがJST当日より前または89日より先のrecordを削除する。さらに部屋指定なしと観測済み7room x 4食事 x 4人数 x 90日を失わない最大11,520件を上限とし、超過時はstay dateと取得時刻の古いrecordから削除する。自動pruneはNext専用storeだけを対象とし、delete UIはこの段階へ含めない。
...
- `価格推移` タブ側にも、既存 `競合価格` タブの graph と同じ形の `部屋タイプ`、`食事` 絞り込み UI を表示する。Nextの部屋タイプ候補は、選択前に取得を開始できるよう`シングル / ダブル / ツイン / トリプル / フォース`を固定表示し、保存済みrecordに他の有効room scopeがある場合だけ共通の正規化順で追加する。食事候補は`素泊まり / 朝食 / 夕食 / 朝夕食`を固定表示する。`部屋タイプ=指定なし` 選択時は、7部屋タイプの全scopeが揃った場合だけspecific record全体を `facility x leadTime` ごとの最安値へ集約し、不完全な途中状態では指定なしrequestのrecordを使う。部屋タイプ選択時は対応する `room_type_options[]` 単独指定requestのrecordだけを使う。`食事=指定なし` 選択時は選択room scopeの保存済み食事タイプ全体を `facility x leadTime` ごとの最安値へ集約し、食事タイプ選択時は該当 `mealType` scopeだけを使う。
- `競合価格` タブと `価格推移` タブの RAU graph tooltip は、列名を `施設`、`部屋タイプ`、`価格`、`前回差分`、`自社との差` に揃える。`施設` cell には色 swatch と施設名を表示し、`部屋タイプ` cell にはその行の最安値として採用した plan または request scope の部屋タイプを表示する。同じ取得日または同じ lead time の自社価格が取れない場合、`自社との差` は `-` と表示する。`前回差分`は現在価格 - 前回価格、`自社との差`は競合価格 - 自社価格として符号を維持し、両タブ共通でプラスを好機の緑、マイナスを注意の赤、0・欠損を中立色にする。色だけを意味の唯一の手掛かりにせず、`+ / -`と金額表記を残し、通常文字として白背景で読めるcontrastを保つ。
...
- `RAU-CP-14`のClassic実装が持つ112 scope background queueは、Nextの取得契約へ引き継がない。Nextは初期表示で`room_type_options[]`なしの4食事 x 4人数を最大16 GET / concurrency 2で補い、利用者が部屋タイプを選択した場合だけ、その1roomの不足する同じ16scopeを同じ上限で補う。複数room、未選択room、週・月・隣接日を自動queueへ入れない。
- 価格推移captureはbooking curve coordinatorと競合価格snapshot queueへ混ぜない。`document.hidden`、別Analyze日付への遷移、施設context変更、価格推移本文の非表示で未完了requestをabortし、同一表示contextでは失敗を自動retryしない。利用者が別の有効roomを選択した場合は、現在の有限batch完了後に最新の選択roomだけを開始してよい。
- 価格推移タブのmetaは、保存済み履歴を先に表示したうえで、`確認中 / 取得中（最大16件） / 本日分を保存 / 本日分は保存済み / 90日範囲外 / 公式側データなし / 保存不可 / 取得失敗`を区別する。未取得roomを選択した場合もfilterを消さず、取得中または該当履歴なしを表示する。全room総数やbackground進捗を表示しない。
- filterだけの変更は保存済みrecordから同期再計算し、specific room batchの保存成功後にそのroomのmodel / graphを更新する。未完了または失敗batchのpartial responseは表示・保存へ混ぜず、標準graphと他の保存済みscopeを維持する。
- 価格推移タブを開いたときは、Classic / Nextの保存済み `price-trend-records` 読み込みとfacility context取得を並行開始してよい。保存済みrecordがある場合の先行graph表示、初期16scopeのsame-day skip、specific room選択時の最大16scope、query契約、`price_trend:v1`、Revenue Assistant write 0を維持する。context取得失敗時は安全側へ停止し、保存済み表示を最新扱いにしない。
...
- `部屋タイプ=指定なし` のgraphは、7部屋タイプ x 4食事 x 4人数のspecific recordがすべて揃うまでは初期の指定なしrecordを使う。利用者選択により一部roomだけが保存された途中状態で、指定なしgraphを部分的なspecific集約へ切り替えない。
- browser-localの `localStorage["revenue-assistant:price-trends:v1:background-fixture"]` は旧Classic background queueの表示品質確認専用であり、Nextの取得・表示契約や合格条件に使わない。Nextはrepo内の合成fixtureで、共通filter、room選択、empty / error、desktop / mobileを外部通信とbrowser-local保存なしで確認する。
