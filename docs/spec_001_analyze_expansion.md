...
- `D-20260807-003`以降の補助表示は、Classicで定着した読む順序をbaselineとする。対象scopeを含む見出し、`表示範囲 / 内訳 / 参考線`のcompact toggle、凡例、`全体`と選択中の`個人 / 団体` chartの順に置き、取得条件、欠損説明、rank変更履歴はchart後の`データ条件とランク履歴`へ初期折りたたみで残す。情報を削除したり、chartより前の大きな説明blockへ戻したりしない。
...
- `D-20260807-003`以降の初期表示は、Classicで定着した見方をbaselineとし、1〜4名をそれぞれ自社 / 競合施設別seriesのchart 4 panelとして同時に表示する。panel titleは`1名 最安値`から`4名 最安値`とし、summary cardと選択中1人数だけのdetail chart、人数切替は持たない。部屋 / 食事filterと施設legendは4 panelへ共通適用し、人数ごとにデータがない場合も他panelを隠さず、そのpanelだけ`対象データなし`とする。
