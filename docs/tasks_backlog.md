...
- 状態:
  - 第一段階source candidateのlocal gate、main同期、Next `0.2.0.27` manual publication、公開artifact / Classic baseline照合、Tampermonkey更新、Top / Analyze各3 controlled runまで完了した。公開Next `0.2.0.26`の100ms / 30 controlでは、cache削除やscope拡張をせず通常操作の可視windowでTop 21件、選択room 44件、別room 3件の計68 booking curve startを観測した。最大同時16、scheduler error / 401 / 403 / 429は0、外部timestampを取った3件の最短開始間隔は107ms、同区間のRevenue Assistant write / HTTP / runtime errorは0だった。`0.2.0.27`の別profile `booking-curve-50ms-20`は保存済みdataで各runが収束し、candidate schedulerのplanned / started / abortedは各0件だった。標準表示、Top exact group 86 / 86・rank 65 / 65、Analyze全6 room readyは維持したが、自然な連続20 startsまたはconcurrency 10、candidate合計50 GETを未観測のため、第一段階は採用済みとも失敗とも判定せず通常利用の次回due windowへ継続する。
- 解決する問題:
...
Remaining Task Triage は、Nowを公開Next `0.2.0.27`の50ms / concurrency 20 candidateで、通常利用から自然にdue requestが発生する次の可視windowを観測するgateとする。Tampermonkey更新、Top / Analyze各3 controlled run、marker / profile一致、標準表示とcoverage維持は確認済みだが、candidate schedulerのplanned / started / abortedは各0件だった。cache削除、scope拡張、強制全取得で負荷を作らず、自然な連続window 20 starts以上またはconcurrency 10以上、candidate合計50 GET、最大同時20以下、HTTP / runtime error、401 / 403 / 429、Revenue Assistant write 0、標準UIとSLO milestone非悪化を確認するまで採用済みとしない。Top cueの今回3 runは100ms / 30 controlよりdescriptive rangeが長かったため、同じwarm / revalidate条件の自然取得windowで再比較する。`RAU-PERF-20`の各cohortは20 sample未満でprovisionalのため、通常利用のTop基準日判断、warm / revalidate、各20 sample収集も並行継続する。35msは50msの安全性とpace律速を確認した後だけ、`RAU-PERF-22`は第一段階後も同一cohortのSLO missまたは3 controlled run以上の同一原因再現がある場合だけ開始する。`RAU-WC-34`のcount-only live range gateは`RAU-PERF-21B`第一段階通過後、通常利用によるdata蓄積後の`RAU-RR-67` fresh再集計・policy確定・backtest / fixed scenario testは独立のAfter Nextとする。現時点の`RAU-RR-67`は7日observed独立3 cluster以上が0 groupのためUI実装no-goであり、compact UIと`RAU-RR-65`一括調整dry-runは各gate通過後に限る。`RAU-WC-34`はrank statusの小chunkとbooking curveの距離別cadenceを分離し、半年全roomの日次取得を採用しない。`RAU-MP-09`の西暦比較表示は利用者確認済みで、次のruntime変更の通常Chrome gateでは月次routeのrequest count / Revenue Assistant write 0を再確認する。Classic再公開、未調査endpoint、current-rank日次snapshot、据え置きcontrol、既存snapshotの更新・削除・一括移行、Revenue Assistant writeはtask進行から推論せず別gateのまま残す。`RAU-UX-145`はNextが旧stacked railを採用していないため再採用せず、同じhost構造を採用する将来変更時だけ再開する。
