...
- GUI 確認:
  ...
  - 2026-05-29 の食事タイプ別実装では、保存対象を `NONE`、`BREAKFAST`、`DINNER`、`BREAKFAST_DINNER` に広げた。通常 Chrome の既存 Revenue Assistant tab は reload 後に `/api/v2/yad/info` が 403 になったため、食事タイプ別 API hash と GUI の再実データ確認は未実施である。ローカル verify は `npm run typecheck`、`npm run lint`、`npm run build`、`git diff --check` を実施した。
