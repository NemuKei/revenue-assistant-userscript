...
- 状態:
  - 2026-07-20 に local 実装、合成 fixture QA、Tampermonkey を無効化した単一 runtime の live candidate QA まで完了した。`RAU-UX-142` の loading shell は維持し、booking curve scoring、lifecycle、対象月、work-state filter が確定した最初の正しい task を先に描画する。current settings だけから仮の方向、rank、reasonFingerprint は出さず、重い reference / graph preview は選択中候補だけを lazy hydrate する。
- 目的:
...
- gate:
  - 利用者は 2026-07-20 に Tampermonkey userscript を無効化し、Revenue Assistant へログインした。candidate は検証後の reload で除去し、通常画面へ戻した。publish、実 write、Tampermonkey の再有効化は含めない。
- local implementation:
  ...
  - 3 か月全件の evidence barrier を、対象月優先・宿泊日順の coverage discovery へ置き換えた。初回は未確認 1 日、後続は最大 7 日を確認し、確定済み候補と evidence を同一 context へ蓄積する。exact dependency は当日 `currentRaw` と `asOfDate` 以降の `sameWeekdayRaw` だけとし、pending / ready / unavailable、same-weekday 不足による incomplete evidence を分ける。bulk IndexedDB read failure は transient pending のまま 1 回だけ browser-local retry し、終端欠損へ誤確定しない。同日 roomGroup 集合が変われば dependency fingerprint でその日だけ commit を無効化する。exact-priority retry は非優先 queue の後ろへ送らず、非同期 queue build は run ID / 対象月で stale 結果を捨てる。queue build failure は settled とみなさず同じ context で 1 回だけ再構築し、pre-scan hit、後続取得、または同じ facility / batch の stale in-flight worker 完了で exact raw が揃った incomplete / unavailable 日は該当日だけ commit / evidence を無効化して再評価する。全日確認前は `候補なし` と断定せず、確認済み日数、見つかった候補総数、実表示件数を分け、対象月 option の部分件数は月ごとの coverage で判定する。既存 warm cache task の集合、request scope / pace / concurrency、retry 回数、run limit は変更していない。
  - live で見つかった標準画面との 2 件の競合を修正した。無関係な calendar mutation generation だけで current settings 完了後の同期を捨てないよう、stale guard を interaction generation、facility / batch / range、host、cell identity に限定した。標準 desktop は 3 か月それぞれ 560px と rail 幅を同時に確保できない場合に全幅 calendar + 後続 rail / detail とし、calendar cue は native date link の `position:absolute` を上書きしない。
- local verification:
  ...
  - post-live hardening 後の fresh local verify は `npm run check`、`npm run build:vite:fixture`、`npm run build:vite:candidate`、`npm run check:fixture-markers`、`npm run check:request-scheduler`、`npm run check:distribution-smoke-fixture`、`npm run check:booking-curve-smoke-fixture`、`npm run build:compare:vite`、`npm run react:doctor -- --diff false --verbose`、`git diff --check` が exit 0。candidate は 652,371 bytes、正規 build の 652,361 bytes との差 10 bytes、metadata mismatch 0、SHA-256 `BEC26BC6FAFFC857FB40AE28D780718707A00A445EC550AC496F9C8B05EE83E4`。React Doctor は 53 warnings で、既知の entry detection、sequential await / `flushSync`、token 名の false positive、既存 unused dependency を含み、blocking error はない。通常 Chrome を安全に終了した後は CDP `127.0.0.1:9222` が利用できないため、candidate の `smoke:distribution` は再実行せず、self-test と直前の単一 runtime live QA を証跡とする。実 DOM の retry focus ring と post-live hardening の再注入は未確認である。
  - 2026-07-20 の live 確認済み candidate SHA-256 は `D924A69D075645E25D8F86EA4E15BDF96603618CFCA2CDD822763418B3E36CF9`。live sanitized timing は shell 30ms、current settings 327ms、最初の candidate evidence 1,051ms、最初の正しい task 1,168ms、選択詳細 evidence 5,774ms だった。候補総数 438、初期表示 10 で、OH / 個人 / 団体、選択維持、console warning / error 0、監視対象 write API POST 0 を確認した。標準画面と candidate の calendar geometry は、calendar 1905 x 747px、各月 603 x 707px、各週 top、date link `position:absolute` が一致した。candidate は reload で除去し、live 値を含む一時 screenshot 5 件は削除した。
...
- 受け入れ条件:
  - 高優先候補を隠さず、cue の意味、rail との対応、対象月切替、表示件数変更、screen-reader description を説明できる。
...
- 状態:
  - 完了。別 identityのNext candidate、基準日レンズ、最終live QA、Classic公開baseline、verify-only workflow、cutover parity matrix、remote dependency / Actions更新の統合、push、post-push proofまで完了した。
- 目的:
...
- 完了済み:
  ...
  - 初期 boundary-only entry で read-only marker と document 単位 runtime lease を先に確立し、既存 Classic DOM または別 runtime lease を検出すると fail closed する契約を確認した。その後の実画面接続 shell も API request、storage、write 操作は開始しない。
  ...
  - merge commit `57c99837303ed07ca86af924c922fcf783a342eb` のpush後、Validate Main run `29888589509`だけがsuccessし、Publish系run 0件、Pages deployment 0件、公開Classicのversion / bytes / SHA不変を確認した。
- 後続gate:
  - 完全な画面内 switch は Classic 側にも共通 guard を入れ、Classic / Next の同時実行を双方から防止できるようにした後の別 gate で扱う。
  - Classic再公開、Next publish、Tampermonkey install / switch、実データ接続、write 操作はそれぞれ別 gate とする。
...
- 状態:
  - 完了。旧版データのread-through、差分計画、保存時merge、速度設定、契約検査、正本同期をlocalで実装・検証し、公開版`0.2.0.3`への配信、Tampermonkey更新後のRevenue Assistant実画面QAまで確認した。
- 解決する問題:
...
- 実画面確認:
  - 2026-08-08に通常Chromeでinstalled runtime `0.2.0.3`、Next単一root、Classic marker 0を確認した。保存済み履歴を起点に不足source 300件を複数の可視sessionで補い、最後の同日sessionは`本日差分完了 0/0`、booking curve GET 0へ収束した。完全観測した79 GETのsessionは開始間隔最小105ms、最大同時10、HTTP error 0、401 / 403 / 429 0、Revenue Assistant write 0、runtime exception 0だった。
  - Topは標準calendarとNext類似日レンズを各1件、Analyzeは標準booking curve 2 chartとNext 2 panel、標準競合価格本文とNext 4 panel、標準価格推移本文とNext 4 panelを維持した。tab切替で前のNext rootはcleanupされ、app console warning / errorは0だった。browserの`unload` permissions-policy message 1件はpage runtime errorと分けた。
- 残る確認 / gate:
  - Classic / Nextの保存元別件数はbrowser storageを直接調べていないため、Classic read-through自体の採用条件とlazy promotionはpure / fixture確認を正とする。翌営業日をまたいだ通常利用で、新しく増えたsourceだけを補うことは継続観察する。
  - Classic再公開、新規endpoint、取得範囲やsession上限の拡張、retention変更、Revenue Assistant writeは配信とlive QAから推論せず、必要な別gateを維持する。
- metadata:
  - `spec-impact: yes`
  - `spec-checkpoint: during-impl`
...
- 実装境界:
  - currentの`0日前`は宿泊日当日までのexact observationだけ、ACTは分離したlandingだけを使う。referenceの`0日前`はtrusted exact、core値、`1日前`とACTの表示専用補間の順で解決し、補間はtooltip / table / ariaで明示して保存しない。季節型は直近型とともに初期表示する。
  - 後続responseから追加するのは既存recordにない有効日付だけとし、保存済みpointを上書きしない。宿泊後に初めて返ったstay-date pointを`0日前`へ戻さない。内部欠損だけを理由にrequest、due task、取得範囲、session上限、concurrencyを増やさない。
  - Analyzeの販売設定とbooking curve runtimeを`load` / `pageshow`でも再同期し、既存の標準DOM監視と重複root除去を維持する。
- 合格条件と結果:
...
- 状態:
  - source、spec、focused / full check、desktop / 390px合成Chrome QA、Next `0.2.0.10`へのmanual publicationまで完了。利用者は更新後の通常Chrome実画面でグラフ色と線種を確認し、Tooltipのpanel clampだけを`RAU-UX-166`へ切り分けた。
- 解決する問題:
...
- 実装境界:
  - booking curve、競合価格履歴、90日価格推移のTooltipをcursorまたはkeyboard選択位置の右へ8px空け、panel端を越えてよく、viewport端だけでclampする。booking curveの中央寄せtransformは外す。
  - 色 / 線種、active guide / point / column、mouse / keyboard / tap、accessible table、panel layout、標準UI、取得、保存、API、retention、Revenue Assistant writeは変更しない。
- 合格条件とlocal結果:
...
- 状態:
  - 完了。source実装、focused / full check、desktop / 390px合成Chrome QA、Next `0.2.0.13`へのmanual publicationを完了し、利用者はTampermonkey更新後の通常Chromeで問題がないことを確認した。
- 解決する問題:
...
- 検証:
  ...
  - 利用者は公開版への更新後、通常Chrome実画面でランク変更グラフとTooltipに問題がないことを確認した。
- metadata:
  - `spec-impact: yes`
  - `spec-checkpoint: before-impl`
  - `target-spec: docs/spec_001_analyze_expansion.md`
...
- 状態:
  - 実装・公開と利用者の通常Chrome表示確認まで完了。`D-20260812-001`のloading先行表示と既存月次response `updated_at` bootstrapをNext `0.2.0.15`へ配信し、比較年を相対語から表示月基準の絶対年へ変えた`0.2.0.16`を公開した。2026-08-12に利用者が西暦比較表示を確認し、問題なしと回答した。runtimeのrequest count / Revenue Assistant write 0を通常Chromeで独立計測する作業は、次のruntime変更のlive gateで月次routeを再確認する。
- 目的:
...
### RAU-PERF-20〜22 Top / Analyzeを判断可能時間SLOで最適化する
...
- metadata:
  ...
  - `risk: instrumentation only; request / storage unchanged`

#### RAU-PERF-21 current優先でbooking curveのhead-of-lineを外す

- 状態:
  - 完了。source実装、local gate、main同期、Next `0.2.0.26` manual publication、公開artifact / Classic baseline照合、Tampermonkey更新、通常Chrome before / afterまで完了した。既存request上限、100ms / 30 control profile、保存・停止契約を維持して5段階queueへ分離し、未選択room referenceをcard openまで遅延した。
- 解決する問題:
...
- 実装scope:
  - booking curve queueを`critical-current`、`visible-current`、`selected-reference`、`visible-reference`、`background`へ分ける。利用者が明示選択したscopeはqueued taskだけpriority upgradeし、同じcontextでactiveなrequestはpriority elevationだけを理由に中断しない。route / facility / as-of / visible-range context変更時の既存abortは維持する。
  - referenceをcurrentと同順位にせず、hotel / selected room currentを描画してからreferenceを段階追加する。全room完了をhotel summaryまたは明示選択roomのbarrierにしない。
  - `RAU-PERF-20`で同じfacility / as-of / rangeのfacility / current settings GET重複が観測された場合は、entry-ownedのin-flight dedupeだけを追加する。route / facility / as-of変更では破棄し、完了結果を別contextへ再利用せず、API queryとrequest上限を広げない。
  - request対象、due判定、100ms以上、concurrency 30以下、bootstrap 800 / daily 200、interactive reserve 32、401 / 403 / 429即停止、連続3 error停止、同一run retryなし、保存schema / retention、Revenue Assistant write 0を維持する。
- 合格条件:
  - synthetic 1 / 6 / 12 / 20 roomで、background active中のhotel current、明示room current、remaining current、selected referenceの開始順、pending dedupe / priority upgrade、route / facility / hidden abortを固定した。販売設定fixtureでhotel current / reference、未選択room current、room open時のselected昇格も確認した。
...
- metadata:
  ...
  - `target-spec: docs/spec_001_analyze_expansion.md`
  - `decision: D-20260813-004`
  - `risk: queue order and deferred unopened-room reference; no endpoint, request cap, storage or retry expansion`

#### RAU-PERF-21B Next booking curveのrequest paceを段階緩和する

- 状態:
  - 第一段階source candidateのlocal gate、main同期、Next `0.2.0.27` manual publication、公開artifact / Classic baseline照合、Tampermonkey更新、Top / Analyze各3 controlled runまで完了した。公開Next `0.2.0.26`の100ms / 30 controlでは通常操作で計68 booking curve start、最大同時16、scheduler error / 401 / 403 / 429 / write 0を確認した。`0.2.0.27`の別profile `booking-curve-50ms-20`は、別の自施設の自然なcold windowでbackground planned / started 768 / 768、最大同時20、約133秒settle、外部timestamp最短51ms、candidate error / 401 / 403 / 429 / write 0を確認し、件数・連続window・負荷安全sub-gateを通過した。cache削除、scope拡張、強制全取得は行っていない。同じsessionのAnalyzeはreserve 32件で全体 / 全roomをreadyにしたが、その後の選択room reference根拠が20秒以内にsettleしなかったため、50ms candidateを維持しつつshared budget事象を追加観測する。controlのTop / Analyze反復は各2 run、各performance cohortも20 sample未満のため、profile採用とSLO達成はprovisionalのままとする。
- 解決する問題:
...
- 合格条件:
  - pure / scheduler fixtureで開始間隔50ms、concurrency 20、budget、pending dedupe、停止後の新規startを固定した。401 / 403 / 429はrunを即停止し、429、連続error停止、設定上限超過、標準UIまたはinteractive latencyの悪化では100ms / 30へ戻せる境界を維持する。
...
- 状態:
  - 条件付き候補。`RAU-PERF-21`と`RAU-PERF-21B`の第一段階後に20件以上の同一cohortでSLO未達、または同じshared budget / queue wait原因による`budget-reached`かinteractive遅延を同一revisionの3 controlled run以上で再現した場合だけ開始する。公開Next `0.2.0.27`のcold Top 768 + Analyze 32でsession上限800へ達した後、選択room currentは23msでreadyだがreference根拠が20秒以内にsettleしない事象を1 controlled run観測した。開始条件には未達であり、20件未満のdescriptive baseline 1回だけではrequest budgetやscopeを増やさない。
- 設計順:
...
- 最初のscope:
  ...
  - eventは施設最大4,096件、coverageは施設最大120件へboundedにし、deterministic key、Web Lock、add constraint、同一transactionのpruneを使う。既存Next / Classic DBをupgrade、更新、削除、一括copyしない。rollback時はwriter / readerを外し、新DBはinertに残す。
  - pure coverage modelで隣接1段変更、週ブロックepisode、facility + reflected date単位の独立decision cluster、3日 / 7日exact transient pickup、再変更censor、同日複数変更、future / 宿泊日超過、欠損を区別する。rank status 0件を変更なしcontrolへ推測しない。
- 合格条件:
  - synthetic fixtureで同一payload再取得の追加0、event / coverage add、施設境界、batch / retention上限、atomic prune、store failure非波及、PII / raw field非保持を確認する。
  - 連続する週ブロックを1 episode / 1 decision clusterとして数え、3日 / 7日exact ready、次変更censor、同日複数変更、2段移動、rank順未確認、欠損、負のpickup、no-change control disabledをpure checkで区別する。
  - 既存calendarのrank status GETは可視範囲最大1のまま、rank sequences / booking curveの追加GET 0、Revenue Assistant write 0、標準UI非干渉をfocused / full checksと通常Chromeで確認する。期待値、確率、推奨金額、一括dry-runは表示しない。
- 進捗:
...
Remaining Task Triage は、Nowを公開Next `0.2.0.27`の50ms / concurrency 20 candidate継続観測とする。自然なcold windowでbackground 768 start、最大同時20、外部最短51ms、candidate error / 401 / 403 / 429 / write 0を確認し、負荷安全sub-gateは通過した。50ms profileはrollbackせず維持する。一方、Top 768とAnalyze reserve 32でsession上限800へ達した後、選択room currentは23msでreadyでもreference根拠が20秒以内にsettleしないshared budget事象を1 run観測した。追加負荷を意図的に作らず、同じ原因を3 controlled runまたは同一cohort 20 sampleで確認した場合だけ`RAU-PERF-22`のbackground pause / bounded critical token設計へ進む。controlのTop / Analyzeは各2 run、各performance cohortも20 sample未満のため、profile採用とSLO達成はprovisionalを維持する。35msは50msの安全性だけでなくpaceが次の律速である証拠が出た場合だけ、`RAU-WC-34`のcount-only live range gateは`RAU-PERF-21B`第一段階の採用判断後とする。通常利用によるdata蓄積後の`RAU-RR-67` fresh再集計・policy確定・backtest / fixed scenario testは独立のAfter Nextとする。現時点の`RAU-RR-67`は7日observed独立3 cluster以上が0 groupのためUI実装no-goであり、compact UIと`RAU-RR-65`一括調整dry-runは各gate通過後に限る。`RAU-WC-34`はrank statusの小chunkとbooking curveの距離別cadenceを分離し、半年全roomの日次取得を採用しない。`RAU-MP-09`の西暦比較表示は利用者確認済みで、次のruntime変更の通常Chrome gateでは月次routeのrequest count / Revenue Assistant write 0を再確認する。Classic再公開、未調査endpoint、current-rank日次snapshot、据え置きcontrol、既存snapshotの更新・削除・一括移行、Revenue Assistant writeはtask進行から推論せず別gateのまま残す。`RAU-UX-145`はNextが旧stacked railを採用していないため再採用せず、同じhost構造を採用する将来変更時だけ再開する。
...
2026-06-05 に、直前の goal bundle で Product Design plugin を使わずに UI/UX 変更を完了したことを、`RAU-UX-126` として task 化した。既存の Product Design audit task (`RAU-UX-106` から `RAU-UX-111`) は `0.1.0.372` 時点の既存 UI surface を対象にしたものであり、今回の `0.1.0.378` 配布版で対象月 filter、月別優先取得、補助操作 `その他`、競合価格 preview 設計、先行月 queue 方針が進んだ後の再 audit とは対象が異なる。`docs/context/INTENT.md` は、表示密度、UI / UX、request 数、安定性の判断に関わるため関連ありとして確認したが、既存の「シンプルで分かりやすい UI / UX」「request 数より安定性」「自動反映より安全な作業キュー」原則で説明できるため更新しない。Remaining Task Triage は Now `RAU-UX-126`、Next / After Next / Later なしとする。今回の task 化では、runtime UI、`src/`、`dist/`、Tampermonkey installed version、Revenue Assistant API request 範囲、Revenue Assistant write API endpoint、rank change payload、booking curve warm cache queue、request 間隔、同時実行数、candidate scoring、priority、confidence、reasonFingerprint、保存 schema は変更していない。
...
- スコープ:
  - Product Design plugin の `get-context` から始め、既存 UI surface の audit / plan first として扱う。
  - 通常 Chrome の `0.1.0.378` 配布版 top 画面、または同等の fixture / screenshot を入力にする。
  - desktop と mobile 390px 相当の表示密度、操作距離、折り返し、横 overflow、button / select / details の視認性、keyboard / focus の懸念を確認する。
  - この task では runtime UI を変更しない。改善が必要な場合は、実装 task を別 ID で作る。
- 受け入れ条件:
  - Product Design の brief / audit 結果が、対象 surface、確認した viewport、主要 findings、残リスク、採用しない案を含んで記録されている。
...
- metadata:
  ...
  - `spec-checkpoint`: before-implementation
  - `target-spec`: `docs/spec_003_rank_recommendation_signal.md`
  - `verify`: Product Design audit note, optional Chrome / fixture screenshot, `git diff --check`
...
- スコープ:
  - 対象は top 料金調整候補 row の `その他` details、primary action、secondary action、pending notice、mobile 390px 相当の重なりである。
...
- 受け入れ条件:
  - 通常利用または配布版 top 確認で、`その他` details の開閉頻度、見つけにくい副操作、pending notice との重なり有無が記録されている。
...
- 完了結果:
  - 競合価格 preview に `roomGroupName x jalanFacilityRoomType` の対応分類を追加した。保存済み snapshot の部屋タイプ label が候補 `roomGroupName` に一意に含まれる場合だけ `confirmed` とし、その label で preview graph を絞り込む。
  - 複数 label が候補になる場合は `ambiguous`、一致 label がない、または snapshot 側に比較できる label がない場合は `unknown` とし、強い絞り込みは行わない。
  - `ambiguous` / `unknown` では preview 内に部屋タイプ対応未確認を明示し、金額推奨や候補方向の主因には使わない。top list 本文や summary へ金額、差額、percent は常時表示しない。
  - 対応分類は表示絞り込み用に限定し、candidate scoring、priority、confidence、reasonFingerprint、Revenue Assistant write API endpoint、rank change payload、request 間隔、同時実行数、保存 schema は変更していない。
  - 正本は `docs/spec_003_rank_recommendation_signal.md` に反映した。
- metadata:
  - `spec-impact`: yes
  - `spec-checkpoint`: during-implementation
...
- スコープ:
  - 対象は `src/main.ts` の `buildRankRecommendationCurveEvidenceByKey()` である。
  - `response.suggest_output_current_settings` と `rm_room_groups` を loop し、visible stay date と `roomGroupId` が有効なものだけ `readRankRecommendationCurveEvidence()` request 配列へ push する。
  - `ownPricePositionEvidenceByStayDate` による stay date 単位の promise reuse と、既存 `rawSourceReader` の共有は維持する。
- 非目標:
  - Revenue Assistant API request 範囲、request 件数、request 間隔、同時実行数は変更しない。
  - `current_settings` response shape、booking curve raw source read key、reference curve、competitor price snapshot、candidate scoring、priority、confidence、reasonFingerprint は変更しない。
  - 保存 schema、Revenue Assistant write API、rank change payload、runtime UI、`dist` は変更しない。
  - `RAU-UX-130` / `RAU-UX-131` の実データ preview / 通常利用観察はこの task では完了扱いにしない。
- 受け入れ条件:
  - visible stay date に含まれない current setting は従来どおり evidence request を作らない。
  - `roomGroupId` が空の room group は従来どおり evidence request を作らない。
...
### RAU-CP-52 forecast diagnostics warning 集約を loop 化する

- 状態:
  - 完了。
- 目的:
  - forecast / evaluation diagnostics の warning 集約で、`flatMap()` や spread によって作る中間配列を減らす。
- スコープ:
  - 対象は `src/curveCore.ts` の `buildUnitPriceForecastResult()` と `summarizeForecastEvaluationResults()` である。
  - observations、case warnings、result warnings を loop し、warning を `Set` へ直接追加する。
- 非目標:
  - warning の重複排除と初出順は変更しない。
  - missingReason、sourceCounts、forecast / evaluation metrics は変更しない。
  - rank recommendation scoring、Revenue Assistant API request 範囲、request 件数、保存 schema、Revenue Assistant write API、rank change payload、runtime UI は変更しない。
  - `RAU-UX-130` / `RAU-UX-131` の実データ preview / 通常利用観察はこの task では完了扱いにしない。
- 受け入れ条件:
  - Unit price forecast の diagnostics warnings は、matching observations の diagnostics を従来どおり初出順で重複排除する。
  - Forecast evaluation の warnings は、各 input の case warnings、result warnings の順に従来どおり初出順で重複排除する。
  - metrics、excludedCaseCount、missingReason、sourceCounts の contract は維持される。
  - `npm run typecheck`、`npm run lint`、`npm run build`、`npm run check:fixture-markers`、`git diff --check` が通過している。
- 完了結果:
...
### RAU-CP-57 curve preview reference source stay date 準備を loop 化する

- 状態:
  - 完了。
- 目的:
  - top 料金調整候補の curve preview reference source を読む前に、preview stay dates から unique compact stay dates を作る際の、skip path 空配列 / 採用 path 1 要素配列を減らす。
- スコープ:
  - 対象は `src/main.ts` の `readRankRecommendationCurvePreviewReferenceSources()` である。
  - `options.stayDates` を loop し、valid compact stay date だけを `uniqueStayDateSet` へ直接追加する。
- 非目標:
  - invalid date skip、dedupe、input 初出順、rawSourceReader に渡す stayDate 集合は変更しない。
  - IndexedDB read 件数、reference curve 計算結果、candidate scoring、priority、confidence、reasonFingerprint は変更しない。
  - Revenue Assistant API request 範囲、request 件数、request 間隔、同時実行数、保存 schema、Revenue Assistant write API、rank change payload、runtime UI は変更しない。
  - `RAU-UX-130` / `RAU-UX-131` の実データ preview / 通常利用観察はこの task では完了扱いにしない。
- 受け入れ条件:
  - valid compact stay date の dedupe 後の順序は従来どおり input 初出順である。
  - invalid stay date は従来どおり rawSourceReader の read 対象にしない。
  - preview reference source の raw source read と curve preview 表示契約は維持される。
  - `npm run typecheck`、`npm run lint`、`npm run build`、`npm run check:fixture-markers`、`git diff --check` が通過している。
- 完了結果:
  - `flatMap()` をやめ、valid compact stay date だけを loop で `uniqueStayDateSet` へ追加するようにした。
  - rawSourceReader に渡す stayDate 集合、IndexedDB read 件数、reference curve 計算結果、Revenue Assistant API request 範囲、Revenue Assistant write API、rank change payload、runtime UI は変更していない。
  - `npm run typecheck`、`npm run lint`、`npm run build`、`npm run check:fixture-markers`、`git diff --check` は通過した。Vite / esbuild 起動系は sandbox 内で `spawn EPERM` になったため、同じ command を昇格して再実行した。
- metadata:
  - `spec-impact`: no
  - `spec-checkpoint`: not-needed
...
- 実装結果:
  - 価格推移 tab 表示時に保存済み record を `readLatestPriceTrendRecordsForStayDate()` で先に読み、存在する場合は visible 16 scope の fetch 前に graph を描画する。
  - 保存済み record 表示中も visible 16 scope の `/api/v1/price_trends` fetch は skip せず、meta に `保存済み表示・再取得中` を出す。
  - fresh 判定がない record を最新扱いにせず、runtime skip は追加していない。
  - `PriceTrendRecord` 保存 schema、`/api/v1/price_trends` query、background 112 scope の補完方針は変更していない。
- metadata:
  - `spec-impact`: yes
  - `spec-checkpoint`: during-impl
  - `target-spec`: `docs/spec_001_analyze_expansion.md`
...
- 受け入れ条件:
  ...
  - `npm run typecheck`、`npm run lint`、必要に応じて `npm run check:fixture-markers` が通過している。
- assessment 結果:
  - 追加の queue separation 実装は行わない。`runPriceTrendFetch()` は visible 16 scope の fetch 完了後に `schedulePriceTrendBackgroundQueue()` を呼ぶため、background 112 scope は visible fetch の後段で始まる。
  - `RAU-PERF-03` の cache-first 表示は visible fetch 前の IndexedDB read に限定し、background queue へ visible scope を混ぜていない。
  - background queue の document hidden / route mismatch / facility mismatch / section absence 停止条件は維持している。
- metadata:
  - `spec-impact`: unknown
  - `spec-checkpoint`: during-impl
  - `target-spec`: `docs/spec_001_analyze_expansion.md`
...
- 実装結果:
  - warm cache queue build 後、`currentRaw` task だけを exact raw source cache key で pre-scan し、保存済み currentAsOf record がある task だけ queue から除外する。
  - exact key は facility、stayDate、asOfDate / batchDateKey、scope、roomGroupId、endpoint、query、schemaVersion を含む `buildBookingCurveRawSourceCacheKey()` に揃えた。
  - `pastAsOf`、`none`、hotel scope hit による roomGroup task、別 roomGroup hit による candidate roomGroup task は除外しない。
  - 除外数は `data-ra-fetch-performance-summary` の `bookingCurve.preScanHitCount` と warm cache status summary の `pre-scan除外 n` で確認できる。
- metadata:
  - `spec-impact`: yes
  - `spec-checkpoint`: during-impl
  - `target-spec`: `docs/spec_001_analyze_expansion.md`, `docs/spec_003_rank_recommendation_signal.md`
...
- assessment 結果:
  - shared scheduler priority は追加しない。既存 `prioritizeSalesSettingWarmCacheQueueForRankRecommendations()` により candidate `currentRaw x roomGroup` task は non-candidate より先に並ぶ。
  - `RAU-PERF-05` の exact pre-scan により、保存済み currentAsOf hit の candidate currentRaw は request なしで除外され、未保存または stale の candidate currentRaw は従来通り queue 先頭側で fetch される。
  - request 間隔 350ms 以上、concurrency 3、hidden default off、active task 非中断、write API 0 件を維持する。
- metadata:
  - `spec-impact`: unknown
  - `spec-checkpoint`: during-impl
  - `target-spec`: `docs/spec_001_analyze_expansion.md`, `docs/spec_003_rank_recommendation_signal.md`
...
- 受け入れ条件:
  ...
  - docs-only の場合は `git diff --check` が通過している。
- 決定:
  - 採用 policy は B: cache は即表示だけに使い、visible scope は常に revalidate。
  - 理由は、`PriceTrendRecord` に batchDateKey がなく、`fetchedAt` threshold や session marker だけで古い価格・在庫を最新扱いにすると誤読リスクが残るため。
  - rollback は visible revalidate always へ戻すこと。将来 network skip を入れる場合は fresh 判定を別 task で確定する。
- metadata:
  - `spec-impact`: yes
  - `spec-checkpoint`: before-impl
  - `target-spec`: `docs/spec_001_analyze_expansion.md`, `docs/context/DECISIONS.md`
...
- 受け入れ条件:
  ...
  - docs-only の場合は `git diff --check` が通過している。
- closeout 結果:
  - `RAU-PERF-01` から `RAU-PERF-07` は完了。runtime 変更は metrics marker、price_trends cache-first visible revalidate、booking_curve exact currentAsOf pre-scan に限定した。
  - shared scheduler priority、price_trends freshness skip、request 間隔短縮、concurrency 増加、Revenue Assistant write API 連動は実装していない。
  - 残 task は live / visual 観察の `RAU-UX-130`、通常利用観察の `RAU-UX-131` とする。
- metadata:
  - `spec-impact`: no
  - `spec-checkpoint`: not-needed
  - `target-spec`: none
...
- 状態:
  - 完了（2026-06-11）。
  - Tampermonkey installed version を公開版 `0.1.0.424` へ更新後、通常 Chrome / CDP で top と price-trends の `data-ra-fetch-performance-summary` を確認した。top では RAU root count 3、React marker yes、top row 10、`pre-scan除外 106`、marker `bookingCurve.preScanHitCount` 106、`candidateCurrentRawSkipped` 10、`candidateCurrentRawFetched` 0、`candidateCurrentRawErrored` 0 を確認した。price-trends では `smoke:distribution --mode price-trends` が pass し、marker で `visibleScopeCount` 16、`backgroundScopeCount` 112、`cacheReadCount` 6、`networkFetchCount` 20、`errorCount` 0、`firstStoredRecordRenderedAt` と `visibleFetchStartedAt` / `visibleFetchCompletedAt` を確認した。marker は `script[type="application/json"]` の textContent に count / timestamp だけを持ち、response body、価格詳細、施設実データ、予約・在庫・顧客情報、Cookie、token、credential は含まないことを確認した。
  ...
  - 2026-06-11 の再試行でも `npm run chrome:pages` は `connect ECONNREFUSED 127.0.0.1:9222` で失敗した。Chrome 拡張 backend は利用可能で通常 Chrome の Revenue Assistant Analyze tab は確認できたが、read-only DOM 確認では `data-ra-fetch-performance-summary` marker 0 件、RAU root 0 件だった。reload による再確認は、別の Chrome 拡張 UI が開いているため automation がブロックされ実施できなかった。次回は Chrome remote debugging port 9222 を起動するか、ブロック中の Chrome 拡張 UI を閉じてから再実行する。
  - 2026-06-11 の goal 再開後に `npm run chrome:debug:default-profile:resume` で CDP 9222 を起動し、`npm run chrome:pages` は成功した。`smoke:distribution --mode top --cdp-connection page` は RAU root と React marker、write API POST 0 件、console / page error 0 件を確認したが、RAU warm cache average starts per second が 0.31 で fail した。`smoke:distribution --mode price-trends --cdp-connection page` は pass し、価格推移 tab / content / overview / panel / SVG と write API POST 0 件を確認した。ただし read-only DOM で `data-ra-fetch-performance-summary` は top / price-trends とも 0 件だった。公開版 `0.1.0.424` には marker 文字列が含まれる一方、現在の通常 Chrome 実行ページには marker 文字列が注入されていないため、Tampermonkey 実行版が marker 対応版ではない、または実行版更新が反映されていない可能性が高い。Tampermonkey dashboard 更新または一時注入は利用者の明示判断が必要なため、この task は未完了のまま残す。
- 目的:
...
- スコープ:
  - 対象は top 画面の料金調整候補 list にある `対象月` filter の選択肢生成、選択状態、候補 list の絞り込みである。
  - 表示する月は、優先取得月カードと同じ基準月から 6 か月分とする。
  - 初期選択、表示上限、preview close、rank change pending 保護は、既存の対象月切替契約を維持する。
  - 優先取得 button 押下後に対象月 filter を押下月へ切り替える既存挙動と矛盾しないようにする。
- 非目標:
  - candidate scoring、priority、confidence、reasonFingerprint、rank change payload、Revenue Assistant write API endpoint は変更しない。
  - booking curve warm cache queue、request 間隔、同時実行数、保存 schema、Revenue Assistant API request 範囲は変更しない。
  - この task では、競合価格グラフ導線やデータ取得ブーストは扱わない。
- 受け入れ条件:
  - `対象月` プルダウンに、優先取得月カードと同じ 6 か月分の選択肢が表示される。
  - 未来月を選ぶと、料金調整候補 list がその月で絞り込まれる。
  - 候補がない月を選んだ場合でも、既存の empty / summary 表示が崩れない。
...
- スコープ:
  - 対象は top 画面の料金調整候補 row、競合価格 snapshot 取得、押下時の preview 表示、取得状態表示、既存 cache 利用方針である。
  - row には、曲線 button と同じように押したときだけ開く競合価格 graph 導線を設計する。
  - graph を開くときに対象 stay_date の競合価格データが未取得または不足していれば取得を開始する。
  - 設計では、取得中、取得失敗、データなし、再取得、同時クリック時の重複取得防止、既存 cache 利用、競合価格 API の request scope を明記する。
  - `RAU-CP-19` では設計までを完了条件とし、runtime 実装は設計完了後に別 task として切り出す。
- 非目標:
  - この task では runtime UI 実装を追加しない。
  - 全候補、全日付、全競合施設の事前取得は行わない。
  - Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
  - raw trace、HAR、request body、response body、Cookie、token、credential、価格や在庫の非公開データは Git 管理へ入れない。
- 受け入れ条件:
  - 競合価格 graph button の表示条件、押下時の対象日、対象 room type、開閉挙動、focus 戻し、mobile 表示方針が明記されている。
...
- スコープ:
  - 対象は競合価格 snapshot の取得 adapter、browser-local cache 参照、同一 `stayDate` の in-flight request dedupe、取得中 / 失敗 / データなしを UI 層へ渡す state である。
  - request scope は押下 row の `stayDate` と、既存 spec で許可された競合価格 graph 範囲に限定する。
  - raw trace、HAR、request body、response body、Cookie、token、credential、価格や在庫の非公開データは Git 管理へ入れない。
- 非目標:
  - この task では row preview UI を完成させない。UI は `RAU-CP-21` で扱う。
  - 全候補、全日付、全競合施設の事前取得は行わない。
  - Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
- 受け入れ条件:
  - 同一 `stayDate` の競合価格 snapshot 取得が cache hit または in-flight dedupe で重複発火しない。
...
- スコープ:
  - 対象は top 料金調整候補 row の `競合価格` preview button、row 直下 preview、loading / empty / failure / retry 表示、`Escape` close、button への focus return、mobile fixture である。
  - 常時 graph 表示ではなく、押下 row だけを展開する。
  - `docs/context/PRODUCT_DESIGN_AUDIT.md` の `Product Design Re-Audit 2026-06-05` の採用 / 不採用判断を前提にする。
- 非目標:
  - 競合価格 snapshot adapter / cache / in-flight dedupe の主実装は `RAU-CP-20` の範囲とする。
  - 金額、差額、percent、forecast 数値、sales / ADR 数値を top list 本文へ常時表示しない。
  - Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
- 受け入れ条件:
  - row の `競合価格` preview button から、対象 row の preview を開閉できる。
  - `Escape` で preview を閉じ、focus が開いた button に戻る。
...
- スコープ:
  - 通常 Chrome の Revenue Assistant top page を対象にする。
  - GitHub Pages published version と Tampermonkey installed version を一致させてから確認する。
  - `npm run smoke:distribution -- --mode top` または同等の CDP / Chrome 確認で、row preview marker、開閉、focus return、監視対象 write API POST 0 件を確認する。
- 非目標:
  - runtime 実装、CSS、保存 schema、request 間隔、同時実行数は変更しない。
  - Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
  - raw trace、HAR、request body、response body、Cookie、token、credential、価格や在庫の非公開データは Git 管理へ入れない。
- 受け入れ条件:
...
- metadata:
  - `spec-impact`: no
  - `target-spec`: none
  - `verify`: Tampermonkey version check, distribution top smoke, Chrome visual / DOM check
...
- スコープ:
  - 対象は booking curve warm cache queue、月別優先取得、候補 list の対象月 filter、配布版 top smoke の throughput 観測である。
  - 現行の `開始間隔 350ms 以上`、`同時実行 3 件以下`、監視対象 write API POST 0 件の安全条件を前提にする。
  - 調査候補は、優先月の queue 割り込み、利用者が選択した対象月の手動ブースト、取得済み月の cache 再利用、対象月変更時の優先度変更とする。
  - 調査結果として、実装する候補、見送る候補、見送る理由、verify 条件を記録する。
- 非目標:
  - 無制限な並列化、rate limit 回避、認証回避、bot 検知回避は扱わない。
  - Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
  - request body、response body、raw trace、HAR、Cookie、token、credential、価格や在庫の非公開データは保存しない。
  - この task では runtime 実装を追加しない。実装は調査で採用案が決まった後の follow-up とする。
- 受け入れ条件:
  - 現行の月別優先取得で、先行月調整時に待ち時間が発生する具体的な場面が記録されている。
...
### RAU-WC-30 対象月 filter 変更時の同月 boost 入口 / queue 優先化を実装する

- 状態:
  - 未着手。
- 目的:
  - 先 6 か月の対象月を選んだ直後に、その月の候補データが未取得で待ち時間が出る場合、利用者が同月の取得を安全に優先できるようにする。
  - request 間隔や同時実行数を増やさず、queue 順序、cache hit、表示中候補の優先化で体感待ち時間を減らす。
- スコープ:
  - 対象は対象月 filter 変更時の同月 boost 入口、booking curve warm cache queue の優先順、表示中候補に一致する `currentRaw x roomGroup` task の優先化である。
  - 現行の `開始間隔 350ms 以上`、`同時実行 3 件以下`、監視対象 write API POST 0 件の安全条件を維持する。
  - `RAU-WC-31` の RAU warm cache 観測分離を verify 候補として使う。
- 非目標:
  - request 間隔短縮、同時実行数増加、非表示中取得の既定 ON 化は行わない。
  - 競合価格 snapshot queue、価格推移 background queue、Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
  - 自動再ログイン、ID / password / Cookie / token / credential 保存は行わない。
- 受け入れ条件:
  - 対象月 filter 変更後、その月の候補 raw source が未取得の場合に、同月の取得を優先する入口または queue 優先化が発火する。
...
- スコープ:
  - 対象は `npm run smoke:distribution -- --mode top` の booking curve request 観測、RAU worker / fallback reason / request source の分類、出力文言である。
  - request body、response body、raw trace、HAR、Cookie、token、credential、価格や在庫の非公開データは保存しない。
- 非目標:
  - booking curve warm cache queue、request 間隔、同時実行数、保存 schema、candidate scoring、priority、confidence、reasonFingerprint は変更しない。
  - Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
- 受け入れ条件:
  - smoke 出力で、ページ全体の booking curve request と RAU warm cache 観測分を区別できる。
...
- metadata:
  - `spec-impact`: no
  - `target-spec`: none
  - `verify`: `node --check scripts/run-distribution-smoke.mjs`, targeted smoke if feasible, `git diff --check`
...
- スコープ:
  - 対象は通常 Chrome / CDP / smoke helper の手順、タブ foreground / background 判定、request 進行と status 進行の観測方法、README または docs への手順記録である。
  - 実アカウントのログアウト操作や破壊的操作は行わない。
- 非目標:
  - 自動再ログイン、ID / password / Cookie / token / credential 保存は行わない。
  - booking curve warm cache queue、request 間隔、同時実行数、保存 schema は変更しない。
  - Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
- 受け入れ条件:
  - 通常 Chrome で hidden-tab 相当をどう作るか、できない場合に何を代替証跡にするかが明記されている。
...
- metadata:
  - `spec-impact`: no
  - `target-spec`: none
  - `verify`: docs check, optional Chrome procedure dry run, `git diff --check`
...
`RAU-UX-116` では、右下固定 warm cache indicator を廃止し、既存の `data-ra-sales-setting-warm-cache-indicator` が残っていても削除するようにした。warm cache と競合価格 snapshot の状態は、top 画面では `候補データ優先取得` strip と料金調整候補 list 直後の画面内 status、Analyze 画面では Analyze 上部候補一覧、全体 summary、販売設定 UI root の近くへ表示する。取得中、完了、skip、失敗、停止、retry 待ち、ログイン確認、権限確認、クールダウンの表示は、右下固定領域ではなく画面内の文脈で確認する。
`RAU-UX-117` では、`候補データ優先取得` strip の host 解決順を変更し、料金調整候補 list がまだ DOM に存在しない初回でも、候補 list の予定 host が解決できる場合は list 直前へ表示するようにした。候補 list host が解決できない場合だけ、入口を消さないためにカレンダー上部へ fallback する。
`RAU-UX-112` では、価格推移 failure / loading / empty / 保存不可 state に次操作を追加した。HTTP 401 は再ログイン、HTTP 403 は権限確認、HTTP 429 は時間を置くまたはクールダウン、HTTP 5xx は時間を置いた再表示、network error は通信状態確認と再表示、89 日より先または公式側データなしは 89 日以内の宿泊日確認、IndexedDB 保存不可はブラウザ保存領域確認として表示する。
`RAU-UX-113` では、Analyze 上部候補一覧を、top からの pending focus がある場合だけ「遷移元候補の確認」と「同日他候補の確認」に分けた。現行維持案は実装が小さい一方で、遷移元候補と比較候補が同じ表に混ざり、利用者が最初に確認する対象を見失う可能性があるため採用しない。分割案は見出し、説明、並び、highlight だけを変え、read-only、candidate scoring、rank change payload、Revenue Assistant write API 非追加を維持するため採用した。
`RAU-UX-114` では、月次実績 fixture に `monthly-empty` と `monthly-partial` を追加し、mobile 390px の compact / empty / partial screenshot を `.tmp/ux-116-117-112-114-115-visual/` に保存した。custom LT booking curve 本体の計算契約、metric 定義、API request 範囲、保存 schema は変更していない。
`RAU-UX-115` では、top fixture の desktop / mobile screenshot で `その他` details、主要操作、pending notice、row actions の重なりと横 overflow を確認した。現行の `その他` details は、主要操作 `Analyzeで確認`、`曲線`、`ランク調整` を常時見せたまま補助操作を折りたためており、mobile 390px でも横 overflow はなかったため、今回の bundle では再配置しない。再評価条件は、通常 Chrome の実運用で `様子見`、`対応不要`、`要点` が見つからず候補処理が止まること、または pending notice と details が重なって取消が見えにくいことを観測した場合である。
Revenue Assistant API request 範囲、Revenue Assistant write API endpoint、rank change payload、booking curve warm cache queue、request 間隔、同時実行数、candidate scoring、priority、confidence、reasonFingerprint、IndexedDB schema、localStorage key、raw source compact schema、保存 schema、Tampermonkey installed version は変更していない。
verify は `npm run typecheck`、`npm run lint`、`npm run build`、`npm run build:vite:fixture`、`npm run check:fixture-markers`、headless Chrome fixture visual check、`git diff --check` を実行した。Vite、esbuild、headless Chrome は sandbox 内で `spawn EPERM` になったため、同じ command または同じ一時 visual check を昇格して実行した。fixture visual check では、`candidates`、`price-trends-failure`、`monthly-compact`、`monthly-empty`、`monthly-partial` の desktop 1280px と mobile 390px で `documentElement.scrollWidth === clientWidth`、価格推移 failure の next action 1 件、月次 compact / partial の main table 1 件、月次 empty の details 1 件を確認した。commit `8bfed8b` を `origin/main` へ push し、Publish Userscript run `26922907647` は success になった。GitHub Pages published version は `0.1.0.373` である。Tampermonkey dashboard は Chrome extension page のためブラウザ安全ポリシーで操作できず、通常 Chrome の既存 Revenue Assistant tab では旧右下 indicator が 1 件残っていた。そのため、Tampermonkey installed version と配布版 smoke は未同期 / 未確認である。
...
Product Design `get-context` brief、対象 surface、capture 手段、desktop / mobile viewport、評価軸、screen audit 結果、共通 UI primitive / fixture / smoke 改善候補は `docs/context/PRODUCT_DESIGN_AUDIT.md` に記録した。
...
- 完了内容:
  ...
  - response status 再観測では、60 秒で booking curve request 29 件、response 28 件、HTTP 200 が 28 件、HTTP error 0 件、監視対象 write API POST 0 件、console error 0 件、page error 0 件だった。この再観測時点の平均開始件数は 0.76 件/秒だったが、これは reload 後の取得対象が少なく、完了済み cache と skip が多い状態だったため、throughput の主証跡にはしない。
- metadata:
  - `spec-impact`: no
  - `spec-checkpoint`: after-implementation
  - `target-spec`: `README.md`
...
- スコープ:
  - 取得前後で比較する値を定義する。候補件数、active candidate 件数、表示中候補件数、raw source 状態、状態 badge の `取得中` / `確認不足` / `根拠あり` の件数差分を候補にする。
  - 表示文言、表示タイミング、表示位置を設計する。表示位置は `RAU-WC-26` で決める候補 list 側の優先取得導線と整合させる。
  - 比較結果を保存するか、画面内の一時 summary に留めるかを決める。
- 非目標:
  - candidate scoring、priority、confidence、reasonFingerprint は変更しない。
  - Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
  - request body、response body、raw trace、Cookie、token、credential、価格や在庫の非公開データは保存しない。
- 受け入れ条件:
...
- スコープ:
  - `smoke:distribution --mode top` の出力または pass / fail 判定に、RAU 発行 `/api/v4/booking_curve` の HTTP status count、平均 request 開始件数、最小 request 開始間隔、worker 数、監視対象 write API POST 0 件を追加する。
  - 未取得 task が十分ある場合の pass / fail 条件と、cache 済みで request が少ない場合の fallback 判定を分ける。
  - request / response body を保存せず、URL、method、status、timestamp、worker 表示、POST count だけを観測する。
- 非目標:
  - HAR、raw Chrome DevTools Protocol trace、request body、response body、Cookie、token、credential、価格や在庫の非公開データは保存しない。
  - Revenue Assistant write API を実行しない。
  - warm cache の request 間隔、同時実行数、worker 数はこの task では変更しない。
- 受け入れ条件:
...
- 実装内容:
  ...
  - 配布版 top smoke `npm run smoke:distribution -- --installed-version 0.1.0.369 --mode top --url https://ra.jalan.net/ --seconds 60 --version-policy fail` は pass した。出力は RAU root count 3、React marker yes、top row 10 件、月別 control 3 件、button 3 件、worker 3/3、booking curve request count 0 件、fallback reason 出力、console error 0 件、page error 0 件、監視対象 write API POST 0 件だった。cache 済み状態のため、未取得 task が十分ある場合の request throughput 判定は live smoke では発火しなかった。
- metadata:
  - `spec-impact`: no
  - `spec-checkpoint`: after-implementation
  - `target-spec`: `README.md`
...
- スコープ:
  - 月別優先取得 button の最適な配置を再検討する。候補は、カレンダー上部維持、料金調整候補 list 上部への移動、カレンダー側と候補 list 側の両方配置、候補 list 側だけに置く案とする。
  - button 押下後に候補 list の `対象月` filter を押した月へ切り替えるか、カレンダー表示月だけを切り替えるか、両方を連動しないかを比較して採用案を決める。
  - 対象月 filter を切り替える場合、表示上限、booking curve preview、rank change preview、rank change pending をどう扱うかを決める。
  - 配布版 GUI 確認条件を決める。
- 非目標:
  - この task では実装を追加しない。実装は採用案が決まった後の follow-up とする。
  - Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は扱わない。
  - candidate scoring、priority、confidence、reasonFingerprint、API request 範囲、request 件数は変更しない。
- 受け入れ条件:
...
- 状態:
  - 完了（2026-06-11）。
  - price_trends overview の構造 signature から background queue 進捗だけを外し、既存 graph / legend / filter / tooltip を保ったまま meta/status text だけを軽量更新するようにした。records、filter、selected room type、selected meal type、empty/error state が変わる場合は従来どおり graph と tooltip を更新する。
- 目的:
...
- 状態:
  - 完了（2026-06-11）。
  - price_trends tab 表示時に `loadPriceTrendRequestContext()` を先に promise 化し、保存済み `price-trend-records` の cache read と並行して進めるようにした。visible 16 scope fetch の直前で同じ promise を await し、requestContext 取得失敗は従来の visible fetch error handling へ流す。
- 目的:
...
- 2026-06-11 に、`RAU-PERF-02` を docs-only assessment として完了した。shared `createIntervalRequestScheduler()` は `requestKey` in-flight de-dupe、FIFO queue、`intervalMs`、`concurrency`、active request 非中断をすでに持つため、初期データ取得高速化 bundle では priority 拡張を入れない。price_trends は専用 background queue、booking_curve は既存 warm cache queue の task order 改善を優先する。shared scheduler を将来変更する場合は、既存呼び出し完全互換、equal priority FIFO、requestKey in-flight de-dupe 維持、interval / concurrency 維持、starvation なし、active task の中断・並べ替えなしを AC にした別 task とする。`docs/context/INTENT.md` は request 数、安定性、安全な作業キューの判断に関わるため関連ありとして確認したが、既存原則で説明できるため更新していない。次の Now は `RAU-PERF-03` で、`RAU-PERF-05` は Next とする。

- 2026-06-11 に、`RAU-PERF-03` を完了した。価格推移 tab 表示時に保存済み `price-trend-records` を先に読み、同じ facility と stay_date の record がある場合は graph を即描画する。保存済み record 表示中は meta に `保存済み表示・再取得中` を出し、freshness 未確定のまま visible 16 scope の network fetch を skip しない。`PriceTrendRecord` 保存 schema、`/api/v1/price_trends` query、background 112 scope、shared scheduler、Revenue Assistant write API は変更していない。`docs/context/INTENT.md` は request 数、表示速度、安定性、安全な作業キューの判断に関わるため関連ありとして確認したが、既存原則で説明できるため更新していない。次の Now は `RAU-PERF-05` である。
...
- 2026-06-05 に、`RAU-UX-126` の完了報告で提示した次視点を `RAU-CP-20`、`RAU-CP-21`、`RAU-CP-22`、`RAU-WC-31`、`RAU-WC-32`、`RAU-UX-127` として task 化した。あわせて、`RAU-WC-27` の完了結果で候補化していた `RAU-WC-30` も正式 task として追加した。競合価格 preview は adapter / cache / in-flight dedupe が UI の前提になるため `RAU-CP-20` を Now、row preview UI を `RAU-CP-21` として Next、配布版 smoke を `RAU-CP-22` として After Next に置く。warm cache は、queue 優先化 `RAU-WC-30` の安全判定に RAU warm cache だけの観測分離が必要なため、`RAU-WC-31` を Now、`RAU-WC-30` を Next、hidden-tab 実測手順改善 `RAU-WC-32` を After Next に置く。`RAU-UX-127` は現時点で runtime UI を変える根拠がなく、通常利用観察で再配置要否を判断する task であるため Later に置く。`docs/context/INTENT.md` は、表示密度、UI / UX、request 数、安定性、安全な作業キューの判断に関わるため関連ありとして確認したが、既存原則で説明できるため更新しない。今回の task 化では、runtime UI、`src/`、`dist/`、Tampermonkey installed version、Revenue Assistant API request 範囲、Revenue Assistant write API endpoint、rank change payload、booking curve warm cache queue、request 間隔、同時実行数、candidate scoring、priority、confidence、reasonFingerprint、保存 schema は変更していない。
