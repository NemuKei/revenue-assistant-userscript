...
- 状態:
  - Next `0.2.0.17`の利用者更新後通常Chromeで、実Analyzeのembedded booking curveへ試験sectionが接続されず表示0件であることを確認した。embedded表示とSales runtimeのrank-order seamを修正し、Next `0.2.0.19`へのmanual publication後、通常Chromeで試験section、方向判定、F5時のloading UI、route cleanup / 再生成を確認した。source監査で両Analyze runtimeが同じrank読取を重複できる経路を確認し、共有coordinator修正版のlocal gate、main同期、Next `0.2.0.20`へのmanual publication、公開artifact / Classic baseline照合まで完了した。利用者更新後のinstalled `0.2.0.20`では、F5がrootなしから`比較準備中`7件を含むloadingを経てreadyへ戻ることを確認した。観測区間にnon-GETはなかったがF5のCDP eventはtruncatedである。安定した同一page再現でSales room curveを2件開くとrank sequencesが2回開始したため、native Sales remountでpending / snapshot / open scopeを保持し、実際の非表示だけcancel、route / context resetだけresetする修正を追加した。focused regressionは通過済みで、再公開、利用者更新後のstable live request budget / 全区間のRevenue Assistant write 0確認を残す。
- 解決する問題:
