...
- 完了メモ:
  ...
  - `RAU-UX-15` では `scripts/check-userscript-version.mjs` を追加し、local `dist`、GitHub Pages 公開版、手入力した Tampermonkey installed version、CDP 接続中の Revenue Assistant tab を確認できるようにした。`main` push 後の GitHub Pages 配布物は `@version 0.1.0.335` であり、Tampermonkey dashboard も `Revenue Assistant Userscript 0.1.0.335` へ更新済みである。`npm run userscript:version-check -- --installed-version 0.1.0.335 --open-url https://ra.jalan.net/` では local version `0.1.0`、published version `0.1.0.335`、installed version `0.1.0.335`、Revenue Assistant URL `https://ra.jalan.net/` を確認した。
...
- 2026-05-30 に、`main` push 後の GitHub Pages 配布物 `@version 0.1.0.335` を確認し、Tampermonkey dashboard も `Revenue Assistant Userscript 0.1.0.335` へ更新した。CDP 一時注入なしの通常 Chrome top smoke では、候補 row 10 件、React marker、対象月 select、表示 mode、表示上限、rank order control、`曲線` preview visible row 1 件、curve SVG 20 件、`rank調整` preview、decision pending cancel、rank pending cancel、監視対象 write API POST 0 件を確認した。
