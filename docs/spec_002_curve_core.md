...
- 現在値と `recent_weighted_90` の差分を使い、`seasonal_component` の final rooms 推定値を補正する単純モデル。first implementation の第一候補にする。ただし、小キャパで 1 室差が大きく効くため、capacity と sourceCount を diagnostics に含める。
...
- `smape`: 小規模 roomGroup でも比較しやすい相対誤差。ただし実績と予測がどちらも 0 に近い場合の扱いを `RAU-FC-02` で明記する。
...
10. 評価 dataset 実装、first forecast model 実装、rank recommendation scoring 接続の順に進める。
