...
- `sales`: Revenue Assistant の booking curve response に含まれる売上合計。通貨単位は Revenue Assistant response の数値単位をそのまま保持し、core logic 内では表示用の丸めや円記号付与を行わない。
- `ADR`: Average Daily Rate の略で、販売単価を表す。Revenue Assistant response の `*_adr` field を第一候補とし、必要な場合だけ `sales_sum / room_sum` から検算値を作る。
...
## Sales And ADR Extension

`RAU-SALES-02` では、`booking_curve_raw_source:v2` に保存済みの sales / ADR fields を、将来の単価予測と売上予測へ接続できる contract として定義した。`RAU-SALES-03` では、ここで定義する adapter と baseline model を pure function として追加し、UI、API 取得、IndexedDB 読み書きから分離する。

### Input Fields

Revenue Assistant の `/api/v4/booking_curve` response では、`all`、`transient`、`group` の各 segment に次の field が存在しうる。

```ts
type BookingCurveSalesAdrFields = {
...
- `undefined` は、compact source に field が存在しないことを表す。対象 response にその field が無い、または保存 schema が古い場合に起きる。
- `null` は、field は存在するが Revenue Assistant 側で値を返せない状態を表す。
- `0` は有効な数値として扱う。売上 0、室数 0、ADR 0 を欠損と同一視しない。
- `NaN`、`Infinity`、負の値は有効値として扱わず、adapter diagnostics に invalid value として残す。
- `two_years_ago_adr` と `three_years_ago_adr` は optional とする。観測済み record に存在しない場合があるため、存在しないことだけで schema 不整合とは扱わない。

### Sales / ADR Observation

sales / ADR adapter の最小出力は次の形にする。
...
type SalesAdrObservation = {
  ...
  adr: number | null;
  adrSource: "api" | "computed" | "missing";
  diagnostics: string[];
};
```

`SalesAdrObservation` は rooms 用の `CurveObservation` を置き換えない。rooms reference curve と rooms-only forecast は引き続き `CurveObservation.rooms` を使う。sales / ADR は、rank response、単価予測、売上予測に使う別 adapter として扱う。

`adr` の決定順:

1. `apiAdr` が有効な非負有限数なら、`adr=apiAdr`、`adrSource="api"` とする。
2. `apiAdr` が欠損し、`sales` が有効な非負有限数、`rooms` が有効な正数なら、`computedAdr=sales / rooms`、`adr=computedAdr`、`adrSource="computed"` とする。
3. `rooms=0` の場合は `computedAdr=null` とし、`zero_rooms_for_adr` を diagnostics に残す。0 室で sales が 0 でも ADR 0 と推測しない。
4. `apiAdr` と `computedAdr` の両方が有効で差分がある場合でも、既定では `apiAdr` を優先する。差分は `api_computed_adr_delta` として diagnostics に残し、後続で閾値を決める。

segment の使い分け:

- `segment="transient"` は、個人向け rank 判断、単価予測、rank response の主入力候補にする。
- `segment="all"` は、施設全体または roomGroup 全体の売上、RevPAR 相当、capacity に対する売上感の補助入力にする。
- `segment="group"` は、団体起因で rooms、sales、ADR が動いたかを分ける diagnostics に使う。初期段階では、group 単体の ADR 上昇または売上上昇を個人向け rank 上げの主根拠にしない。

### Unit Price Forecast Candidate

単価予測は、rooms-only forecast とは別の候補 model として扱う。first wave では UI へ直接表示せず、売上予測または rank response evaluation の補助入力にする。

```ts
type UnitPriceForecastV1Candidate = {
  modelId: string;
  modelVersion: string;
  facilityId: string;
...
- `api_current_adr_baseline:v1`: `asOfDate` 時点で観測できる `this_year_adr` を `predictedAdr` として返す。単価予測の評価 baseline として使う。
- `historical_adr_reference_baseline:v1`: 同じ raw source 内の `last_year_adr`、取得できる場合は `two_years_ago_adr`、`three_years_ago_adr` を平均して `referenceAdr` とし、現在 ADR が欠損した場合の比較候補にする。ただし、optional field が存在しない場合は欠損として diagnostics に残し、現在 ADR と混ぜて確定値のように扱わない。

### Sales Forecast Candidate

売上予測は、rooms forecast と unit price forecast を合成する model として扱う。sales forecast が rooms forecast を内部で再計算しないよう、入力と責務を分ける。

```ts
type SalesForecastV1Candidate = {
  modelId: string;
  modelVersion: string;
  facilityId: string;
  targetStayDate: string;
...
1. `/api/v4/booking_curve` raw source を `booking_curve_raw_source:v2` として保存する。これは `RAU-RR-02` で完了済みであり、保存単位を追加変更しない。
2. sales / ADR adapter を追加し、`SalesAdrObservation` を作る。ここでは UI へ表示しない。
3. `UnitPriceForecastV1Candidate` を pure function として追加する。`RAU-SALES-03` の初期実装は `api_current_adr_baseline:v1` に限定し、合格基準を固定しない。
4. `SalesForecastV1Candidate` を pure function として追加し、`ForecastResultV1Candidate.predictedFinalRooms` と `UnitPriceForecastV1Candidate.predictedAdr` を掛け合わせる。どちらかが欠損なら `predictedSales=null` とし、欠損理由を diagnostics に残す。
5. rank recommendation へ接続する場合は、数値を top list へ直接表示せず、ADR 悪化、sales 悪化、RevPAR 相当悪化のような非数値 reason / diagnostics から段階導入する。

評価指標:

- 単価予測は `maeAdr`、`smapeAdr`、`biasAdr` を候補にする。
- 売上予測は `maeSales`、`smapeSales`、`biasSales` を候補にする。
- rank response と接続する場合は、rooms pickup だけではなく、ADR change、sales change、RevPAR 相当 change を合わせて見る。
- 合格基準はこの task では固定しない。実装後に実データ分布を見てから判断する。
...
- `0日前` に Revenue Assistant API 側の実績上書きが混入している疑いがある場合でも、core logic 内で中間補完値へ置き換えない。表示層でtrusted exact aggregateを使えるのは、exactとACTの対象stay date集合が一致する場合に限る。集合が異なる場合は母集団の異なるcore数値も終端表示へ採用せず、`1日前`とACTの中間を表示専用補間として描画できる。補間値はcore outputやderived cacheへ戻さず、tooltip、全値表、aria labelで明示する。
...
- `seasonal_component` は、呼び出し側が宿泊後のlandingをstay date別final roomsとして渡せる場合、その値を比率の分母とfinal rooms推定値に使う。landingが渡されない従来callerでは、各stay dateの最小非負LTの観測値を従来どおりfinal rooms候補にする。
- `seasonal_component` の表示では、coreが返す比率1.0の`0日前`とACTを同じfinal rooms推定値として維持し、別母集団のexact aggregateで`0日前`だけを上書きしない。coreの`0日前`が欠損する場合だけ、表示層が`1日前`とACTから補間値を作れる。補間値はcore outputへ戻さず、補間であることを明示する。
...
12. `RAU-FC-05` で、rank recommendation scoring へ forecast diagnostics を接続する。
13. `RAU-SALES-02` で、sales / ADR adapter、単価予測 candidate、売上予測 candidate の契約を確定する。
14. `RAU-SALES-03` で、sales / ADR adapter と `api_current_adr_baseline:v1`、`rooms_x_unit_price:v1` を pure function として実装する。
15. 後続 task で、rank recommendation へ ADR / sales health diagnostics を段階接続する。
