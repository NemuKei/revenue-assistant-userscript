...
- 実データ adapter が未接続の段階では OH、個人、団体、競合をそれぞれ `未接続` と表示し、DOM の標準数値を OH と読み替えたり、値、類似候補、一致度を推測したりしない。
...
| Analyze | 保持、入口再設計 | 全部屋タイプと詳細根拠を確認する深掘り画面として残す。基準日または類似日から対象日へ遷移し、旧候補表は再現しない | Revenue Assistant標準の日付→Analyze導線はlive観測済み。Classic追加機能はsource / smoke evidenceで、Nextから対象日を渡す入口は未実装 | 対象日が維持され、販売設定、booking curve、競合価格、価格推移へ到達できる |
...
| OH / 個人 / 団体 | 必須再接続 | 3軸を別の直接取得値として表示し、0、未取得、未接続を区別する。`all - group`などの差し引きで推測しない。競合も第4軸として分離する | Next adapterは未接続。現shellは4軸を個別に`未接続`表示 | source / as-of / room typeが一致するadapter、missing / zero fixture、4軸説明と類似度根拠が一致する |
...
- live shell の OH / 個人 / 団体 / 競合 adapter は未接続で、実画面では4軸とも`未接続`である。
- 2026-07-22のread-only live QAではAnalyze、競合価格、価格推移、booking curveへの既存導線を確認したが、Classic userscriptを再有効化した比較や実writeは行っていない。
