...
| Analyze | 保持、入口再設計 | 全部屋タイプと詳細根拠を確認する深掘り画面として残す。基準日または類似日から対象日へ遷移し、旧候補表は再現しない | Nextは表示中calendarの確認済みhrefを再利用し、hrefを持たない実画面ではURLを推測せず標準日付clickへ委譲する。候補内buttonから対象日を維持したAnalyze遷移をlive確認済み | 販売設定、booking curve、競合価格、価格推移の各詳細機能をNext cutover前に個別確認する |
