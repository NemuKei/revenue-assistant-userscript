...
- Analyze導線は対象日の確認済みhrefを再利用し、実画面のようにhrefがない場合だけ標準日付clickへ委譲するが、Nextからbooking curve / 競合価格 / 価格推移の各tabを直接開くdeep linkではない。好評な90日価格推移graphを含むAnalyze側parityは維持対象であり、今回のcalendar badge実装でgraph仕様を変更していない。
