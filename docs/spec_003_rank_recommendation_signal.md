...
トップカレンダーの badge だけでは、RM が「何から作業するか」を決めにくい。一方で、カレンダーは日付と曜日を位置で把握でき、既存業務との認知差が小さい。2026-07-17 以降のトップ画面は、既存カレンダーを左の空間的な基準として残し、右の `今日の判断` rail で優先度の高い `stayDate x roomGroup` を選び、下の選択詳細で根拠と次操作を確認する decision workspace とする。これは、カレンダー下へ 9 列の候補 table を置く旧レイアウトを置き換える。
...
- トップ画面に、既存カレンダー、`今日の判断` rail、選択中候補の詳細を組み合わせた料金調整 decision workspace を出す。
- 候補単位は原則 `stayDate x roomGroup` とする。
- rail の候補には、宿泊日、部屋タイプ、現ランク、候補ランク、判断方向、根拠取得状態を出す。選択詳細には、優先度、確度、現在 / 候補ランク、`OH n / キャパ m`、`個人 n`、`団体 n`、判断根拠、注意、更新状況、Analyze 導線を出す。
...
- OH、キャパ、個人、団体のいずれかを直接取得できない場合は `未取得` と表示し、別の値から差し引いて推測しない。0 は欠損と同一視せず `0` と表示する。
...
RAU が top decision workspace から単一候補の rank 変更を実装する場合、対象は rank 変更だけに限定する。推奨金額、人数別価格、食事条件別価格、プラン別価格、販売停止、在庫、複数行選択、bulk apply、自動反映は対象外である。
...
2026-07-17 以降の UI は、候補 task やカレンダーから直接送信しない。選択詳細の `変更内容を確認` で最終確認 region を開き、対象宿泊日、roomGroup、現在 rank、変更後 rank、主要根拠、注意、送信不可理由を表示する。送信不可理由がある場合は最終操作を disabled にし、POST を発生させない。
...
この二段階確認は、旧 UI の `反映する -> 5 秒 pending -> 自動送信` を置き換える。送信前取消を維持するが、RAU が送信後 rollback または undo を提供することは意味しない。`様子見` と `対応不要` の browser-local decision に対する 5 秒 pending は別契約として維持する。
...
#### Superseded Top List Contract

以下は 2026-07-17 より前のカレンダー下 9 列 list / row preview 契約を履歴として残すものであり、現行 layout、work-state control、rank 変更の操作順には適用しない。candidate scoring、data source、lifecycle、API adapter、安全 guard のうち上の現行契約と衝突しない部分だけを維持する。
...
- 出力は scoped `data-ra-*` selector、button attribute、link href、aria attribute、evidence placeholder DOM とする。現行 workspace の smoke selector を正とする。
...
2026-07-17 workspace overhaul 契約:
