...
20件はSLO complianceを主張するための母数であり、計画due task集合とsession capを変えない可逆なpace試験の開始条件ではない。`RAU-PERF-21B`は100ms / 30 controlの後、通常利用由来の50ms / 20でcold Top 768 start、最大同時20、HTTP / runtime error、401 / 403 / 429、Revenue Assistant write 0の連続windowを確認した。`RAU-PERF-23`の35ms / 30はこの50ms / 20をTop backgroundに維持したままfinite foregroundだけへ適用し、別request profileとする。controlとcandidateのsampleは同じp95母集団へ混ぜず、source revision、request profile、room数band、warm / revalidateを分けて比較する。
