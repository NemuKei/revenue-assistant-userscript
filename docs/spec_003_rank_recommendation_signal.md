...
- lifecycle filter と表示モード適用後に表示上限を超える active candidates がある場合は、summary に `他 n件` を表示する。これは現在の表示上限の外にも確認候補が残っていることを示すための件数である。`さらに表示` と `10件に戻す` は表示件数だけを変更する入口であり、candidate scoring、sort、表示モード以外の filter 条件、rank order、manual override、user decision、resolved 判定、API request 範囲、request 件数、推奨金額表示は変更しない。
