...
| Revenue Assistant標準calendarの黒い値 | 保持 | 標準DOM、文字、位置、focus、highlightを変更しない。黒い値をOHへ読み替えず、Next由来のOH / 個人 / 団体とは別契約にする | Revenue Assistant標準画面でlive観測済み。Next shellの非干渉をlive QA済み | desktop / 狭幅 / 再描画後に標準geometryと既存値が不変 |
| Classicが追加する青い`団n` badge | 保持、再構築 | カレンダー上でOH相当の既存黒値と団体数を視覚的に切り分ける役割を残し、団体は直接取得値だけを青い`団n`表記で示す。標準DOMを上書きせず、0、未取得、非表示を混同しない | Classic実装 / sourceは存在。Nextでは未実装 | user指定の表記、直接取得値、表示toggle、再描画、狭幅、0 / 欠損をfixture / live QAで確認 |
| Analyze | 保持、入口再設計 | 全部屋タイプと詳細根拠を確認する深掘り画面として残す。基準日または類似日から対象日へ遷移し、旧候補表は再現しない | Nextは表示中calendarの確認済みhrefを基準日 / 候補日 / 比較cardへ再利用する。fixtureでhrefを固定し、Revenue Assistant標準の日付→Analyze導線はlive観測済み | 対象日が維持され、販売設定、booking curve、競合価格、価格推移へ到達できる |
| booking curve | 保持 | 全体、個人、団体、reference、rank marker、tooltip、ACT空表示を機能契約として維持する。Nextでは選択中日から必要時に開き、全日分を常時展開しない | Classic実装 / smokeは存在。Next入口とcutover contractは未実装 | series切替、0 / 欠損、tooltip、rank marker、失敗時の次操作をfixture / smokeで確認 |
| 競合価格 | 保持、入口再構築 | 保存済みsnapshot、人数別graph、部屋タイプ対応の注意を再利用し、選択中日から必要時だけ開く。競合価格だけで候補方向や推奨金額を断定しない | Classic実装 / snapshot storeは存在。Next入口とrow preview相当のcontractは未実装 | snapshot / empty / error、人数別series、部屋タイプ対応note、同日重複取得防止をcontract testで確認 |
| 価格推移graph | 保持 | 好評な90日graph、人数別series、filter、tooltip、failure時の次操作をAnalyzeで維持する。Nextカレンダーへの常時埋込みは初期cutover条件にしない | Classic実装 / smokeは存在し、実画面導線を観測済み。Next入口は未実装 | graph / empty / error、filter、tooltip、横overflow、console error 0をsmokeで確認 |
| OH / 個人 / 団体 | 必須再接続 | 3軸を別の直接取得値として表示し、0、未取得、未接続を区別する。`all - group`などの差し引きで推測しない。競合も第4軸として分離する | Next read-only adapterとmissing / zero / partial / error fixtureを実装済み。staleはproductionで探索しない過去as-ofの防御表示を合成fixtureだけで確認する。同一room groupだけを比較し、競合はroom type未確認としてscoreから除外する。live response結果の確認は認証session失効で未完了 | source / as-of / room type一致、実画面の4軸状態、類似marker、GET予算、再描画時再利用をlive QAする |
...
- live shellのread-only adapter、cache key、evidence、view model、UIは接続済みで、sanitized fixtureではready / missing / zero / partial / stale / errorを確認した。ただし2026-07-22の実画面QAはcandidate注入と注入時GET 0まで成功した後、最初の選択操作時に認証sessionがloginへ戻ったため、実responseによる4軸表示とGET 2件の確認は未完了である。ページ遷移でcandidateは除去され、candidate endpoint GET 0、Revenue Assistant API POST 0だった。
- Analyze linkは対象日の確認済みhrefを再利用するが、Nextからbooking curve / 競合価格 / 価格推移の各tabを直接開くdeep linkではない。好評な90日価格推移graphを含むAnalyze側parityは維持対象であり、今回のadapter実装でgraph仕様を変更していない。実画面の再確認は認証復旧後に行う。
...
- 実画面screenshotは施設の非公開データを含むため保存していない。したがって、chartの視覚比較、hover依存、keyboard、screen readerの完全な監査はcutover前に別途必要である。
- 現行testは主要markerとsmokeを持つが、競合価格row preview、rank guardの全停止理由、様子見 / 対応不要の保存・filter・再表示をNext contractとして直接固定するtestが不足している。
