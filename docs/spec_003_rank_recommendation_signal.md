...
| booking curve | 保持 | 全体、個人、団体、reference、rank marker、tooltip、ACT空表示を機能契約として維持する。Nextでは選択中日から必要時に開き、全日分を常時展開しない | Next休止中も標準booking curve、全部屋タイプ一覧、2 chartは残る。Classic固有のreference、rank marker、keyboard tooltip hitboxはNextでは未接続 | `RAU-UX-150`で標準chartを複製せず固有補助だけをroute-scopedに再接続し、series / 0 / 欠損 / tooltip / rank markerを確認 |
...
- 標準Analyzeのbooking curveは全部屋タイプ一覧と2 chart、競合価格は人数1〜4 / 食事 / 部屋タイプ条件、価格推移は3 filter、84〜0日前軸、更新時刻、1 graphをNext休止中も維持した。一方、Classic固有のbooking curve reference / rank marker、競合snapshot履歴graph、90日価格推移の人数別4 panelは存在しない。parity確認の結論はcutover可ではなく、`RAU-UX-150`をblockerとして分離することである。
