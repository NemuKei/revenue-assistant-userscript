...
### Next First Phase Candidate

Next first phase の候補では次を行う。Classic への公開済み変更を意味しない。
...
`warm`は必要なexact current sourceが操作開始時にNext storeまたは有効なClassic read-through seedへ存在する状態、`revalidate`は必要なcurrent sourceの一部が未保存またはdueである状態を指し、どちらにもsourceがないcold取得もこのcohortへ含める。revalidateでは、同じfacility / stay date / scopeの有効なlast-known sourceをnetwork完了前に描画してよいが、`stale-revalidating`と利用者へ示し、decision-ready milestoneへ数えない。保存済みsourceがないcold scopeは0やfreshへ読み替えず、取得完了したscopeからprogressiveに描画する。`overallSettled`、`allRoomSummarySettled`、`selectedRoomCurrentSettled`は、対象currentのdueが0で既存のexact current条件も満たした場合だけreadyとする。施設全体の空storeから行うTop bootstrapはbackground準備であり、Analyze SLOの完了条件へ含めない。Top backgroundだけをbootstrap 800 / daily 200で止め、Analyze foregroundはこの件数counterを消費せず固定request-count ceilingを持たない。優先順はAnalyze overall current、利用者が開いたroomのcurrent、同roomのreference / rank marker、1〜12 room summary、Topの画面外backgroundとする。Analyzeの有限due taskがpending中は新しいbackground requestを開始せず、priorityを上げただけでは同じcontextで開始済みのrequestをpreemptしない。Top自動backgroundは50ms以上 / concurrency 20以下、AnalyzeまたはTopの明示操作によるfinite foregroundは35ms以上 / concurrency 20以下とする。due / dedupe、auth / rate-limit / error停止、context / hidden abortは全taskで共有する。route / facility / as-of / visible-range context変更、document hidden、runtime停止では既存契約どおりqueued / in-flightをabortできる。
...
20件はSLO complianceを主張するための母数であり、計画due task集合とsession capを変えない可逆なpace試験の開始条件ではない。`RAU-PERF-21B`は100ms / 30 controlの後、通常利用由来の50ms / 20でcold Top 768 start、最大同時20、HTTP / runtime error、401 / 403 / 429、Revenue Assistant write 0の連続windowを確認した。`RAU-PERF-23`は公開Next `0.2.0.30`でfinite foregroundだけを35ms / 30へ進め、連続取得中の操作応答悪化を受けて`0.2.0.31`で35ms / 20へ分け直した。最大同時20でもuncached room差分後に最大3,927msのLong Taskを再現したため、request profileは維持し、scope別の差分model / DOM更新へ進める。Top backgroundの50ms / 20は維持する。controlと各candidateのsampleは同じp95母集団へ混ぜず、source revision、request profile、room数band、warm / revalidateを分けて比較する。

Nextの計測はversionedな`data-ra-fetch-performance-summary`をDOM上に1件だけ置き、`performance.now()`起点のelapsed ms、固定enum、件数だけを保持する。Topはroute / shell / cached group / rank settled / base decision、Analyzeはsurface / shell / overall / selected room current / selected room evidence / all summary、競合価格はsurface / cache paint / fresh settled、schedulerはinteractive queued / started、background planned / started / settled、aborted / error / stopを区別する。現行schedulerはinteractive admissionの次slotでbackgroundを再開するため、実際にpauseしない状態を`background pause`として記録しない。件数は`eligibleVisibleDates`、`validExactGroupSourceDates`、`renderedExactGroupDates`、`validRankEventDates`、`renderedRankEventDates`、`requiredRoomScopes`、`readyRequiredRoomScopes`、planned / started / aborted request、error / stopだけを許可する。`source`は`cache / network / mixed / none`、`outcome`は`ready / partial / empty / error / aborted`、`freshness`は`fresh / stale-revalidating / unknown`の固定値に限定する。`rau-next-performance-v2`ではこれに加え、booking curveの`responseRead`、`responseParse`、`responseCompact`、`sourceRead`、`sourceBuild`、`sourceWrite`、`storedNotify`、`acquisitionPublish`、`referenceRead`、`curveBuild`、`curveRender`、`markerPublish`だけを固定phaseとし、operation内のcount / total ms / max msを集計できる。Long Taskはobserver状態`active / unsupported / error`とcount / total ms / max msだけを持ち、raw entryやattributionを保持しない。phase更新はmemory内で行い、各requestでmarker DOMを更新せず、終端milestoneまたはcoalesced flushだけで反映する。施設ID / 名、stay date、roomGroup ID / 名、URL、価格、在庫、request / response、storage key、Cookie、token、credentialはmarker、console、storage、docsへ残さない。markerはcontext変更でresetし、永続保存せず、console出力は既存debug flagが有効な場合だけ許可する。p95集計はuserscript内へ履歴保存せず、明示実行したQA collectorが各single-run markerをprocess memoryへ集め、そのcollection windowのsource revision、schema、cohort、sample数、p95 / median / max、coverage、除外理由だけを出力する。通常利用をまたぐ20件が揃わない間はprovisionalを維持する。
...
| 様子見 | 保持、状態再構築 | 一時保留として日付 x 部屋タイプ x 根拠へ紐づけ、期限またはpriority / confidence / pace / OH / 競合 / reasonFingerprintの重要変化で再表示する | Classic decision storeは存在。Next persistence / lifecycle / 直接testは未実装 | cooldown、重要変化、同一根拠抑制、保存失敗時の非反映を直接testする |
| 対応不要 | 保持、意味を明確化 | 様子見と統合せず、`この根拠では対応不要`として扱う。同じreasonFingerprintを抑制し、根拠、方向、confidence段階が変われば再候補化する | Classic decision storeは存在。Next filter / 再表示 / 直接testは未実装 | 保存 / filter / 再表示、様子見との非混同、理由を利用者が確認できる |
| 自動反映 / 一括反映 / 推奨レート金額 | parity対象外 | Next first phaseでは実装しない。将来もuser-confirmedな別仕様 / 別gateとする | 意図的に未実装 | read-only探索と単一候補guardが実データで安定した後に再判断 |

### Current Evidence Limits

- live shellのread-only adapter、cache key、evidence、view model、UIは接続済みで、sanitized fixtureではready / missing / zero / partial / stale / errorを確認した。ログイン済み実画面でも基準日未選択GET 0、選択時の確認済み2 endpoint各1回、facility guard、4軸、類似marker、部分比較、標準表示切替時の追加GET 0、監視対象write 0、cleanupを確認した。
- calendarの青い`団n`はClassicと同じhotel scopeの団体直接値であり、room group値の合算ではない。Nextはcurrent as-ofのfacility / stay date / hotel scope / endpoint / exact queryが一致する既存primary keyだけを基準日選択後にreadonlyで読み、0は表示、欠損 / stale / 不一致は非表示にする。実画面で92日分のbadge、標準値非干渉、desktop / 390pxの重なり0、標準表示切替時の維持、calendar差替え時の破棄を確認した。
- Analyze導線は対象日の確認済みhrefを再利用し、実画面のようにhrefがない場合だけ標準日付clickへ委譲する。calendar lensはproductionの`/`だけで動かし、Analyzeでは選択 / in-flight根拠を破棄してroot、style、badge、marker、追加accessibility属性を除去する。fixtureでは選択済み状態からAnalyzeへ移った後に追加artifact 0、標準3 tab維持、calendar復帰時idle再mountを確認した。
- ログイン済み実画面へNext candidateを一時注入し、通常日付clickで対象日`/analyze/2026-08-01`を維持した。Analyze上はNext root / style / badge / marker 0、candidate起点request 0、Revenue Assistant write API POST 0、candidate / Revenue Assistant由来console warning / error 0、runtime exception 0だった。外部New Relicのerror-report request失敗1件はcandidate起点ではなく、同一originのconsole errorには数えない。reload後はNext runtime属性も0に戻した。
- 標準Analyzeのbooking curveは全部屋タイプ一覧と2 chartを維持し、Next第三段階Aは既存raw cacheのexact primary keyだけからreference比較をnative末尾へ追加する。合成fixtureではready系列、liveでは対象日のexact cache不足によるemptyを確認したため、実画面ready値は未確認である。rank markerと90日価格推移の人数別比較は未接続であり、parity確認の結論は引き続きcutover不可である。
- 390pxではNext artifactは0だがRevenue Assistant標準Analyze自体に810pxの横overflowがある。Next由来の回帰ではないため今回標準DOMを変更しないが、`RAU-UX-150`の追加rootはviewport内に収め、標準overflowを悪化させない。
- 競合snapshotはfacility / stay date単位の保存有無と、その選択日に読めたrecordの取得時刻だけを表示できる。表示期間内の別日にsnapshotがあっても選択日を`保存値あり`にしない。Revenue Assistant room groupとの対応が未確認なので、競合金額、room type別series、類似scoreへの利用は未接続である。
- 実画面screenshotは施設の非公開データを含むため保存していない。route isolationの視覚証跡は合成fixtureだけで確認し、chartのhover、keyboard、screen readerの完全な監査は`RAU-UX-150`へ残す。
- `check:next:analyze-parity`はroute isolationとClassic固有chart contractを固定し、配布版smokeには`competitor-prices` modeを追加した。競合価格row preview、rank guardの全停止理由、様子見 / 対応不要の保存・filter・再表示は引き続き別のNext contractが必要である。

### Non-goals For Next First Phase
...
- current rank が推定順序の端にあり、`raise_watch` または `lower_watch` の隣接 rank が存在しない場合、workspace の task または選択詳細には `上限ランク: 上げ余地なし` または `下限ランク: 下げ余地なし` を表示する。これは推奨レート金額や 2 段階以上の rank 移動を出すものではなく、隣接 rank がない理由を利用者に見せるための表示である。
...
- したがって、RAU からトップ画面で rank 変更を実装する場合でも、標準 UI と同じ write endpoint をすぐ呼ぶのではなく、少なくとも変更内容の明示確認、現在 rank 再取得、対象 `stayDate x roomGroup` の固定、隣接 rank 限定、送信前取消、送信結果の成功 / 一部失敗 / 失敗記録、同時更新検知、実 POST の低リスク確認を別 task で満たす必要がある。
...
Classic の単一行 rank 調整は parity 対象として維持する。Next の top surface へ再接続する場合も、対象は単一候補の rank 変更だけに限定する。推奨金額、人数別価格、食事条件別価格、プラン別価格、販売停止、在庫、複数行選択、bulk apply、自動反映は対象外である。
...
2026-07-17 の未公開 Next workspace 案では、候補 task やカレンダーから直接送信しない。選択詳細の `変更内容を確認` で最終確認 region を開き、対象宿泊日、roomGroup、現在 rank、変更後 rank、主要根拠、注意、送信不可理由を表示する。送信不可理由がある場合は最終操作を disabled にし、POST を発生させない。この二段階 UI は prior design evidence であり、Classic の現公開操作を置き換えた契約ではない。

最終確認 region だけに `この内容で変更する` と `確認をやめる` を表示する。`変更内容を確認` は確認表示を開くだけであり、押下後に待っても countdown や timer で POST しない。`この内容で変更する` の明示押下 1 回だけが送信前 guard を開始できる。`確認をやめる`、reload、施設切替、batch 切替、script 再実行では送信しない。

この二段階確認を Next で採用する場合は、Classic の `反映する -> 5 秒 pending -> 自動送信` から明示的に migration する。送信前取消を維持するが、RAU が送信後 rollback または undo を提供することは意味しない。`様子見` と `対応不要` の browser-local decision に対する 5 秒 pending は別契約として維持する。
...
候補生成後の rank change status 判定には、選択詳細が参照する候補の `generatedAt` を使う。最終確認を開いた時刻や `この内容で変更する` を押した時刻だけを基準にすると、候補表示から送信操作までの間に Revenue Assistant 標準 UI または別端末で行われた rank 変更を見逃すためである。
...
- 送信開始 guard。最終確認 region の `この内容で変更する` が明示押下された場合だけ POST 処理へ進む。確認 region を開いただけ、待機、reload、施設切替、batch 切替、script 再実行では送信しない。POST は 1 候補につき 1 回だけ実行し、自動 retry はしない。
...
次は確認済み仕様として扱わず、現在利用可能な browser / API discovery capability を確認した調査 task、または対象を固定した低リスクの手動確認 task にする。
...
- `対応不要` の判断時より confidence の表示段階が上がった場合は、同じ reasonFingerprint でも再表示候補にできる。ここでいう confidence の表示段階は、workspace に出す `高`、`中`、`低` の段階であり、小数値の細かな増減は直接表示しない。
...
- 同じpayload / visible rangeの再取得はdeterministic keyとIndexedDB `add` constraintで重複保存しない。施設単位Web Lock内の1 readwrite transactionでevent / coverageをaddし、施設最大4,096 event、coverage最大120 recordを超えた古いNext recordだけをpruneする。保存batchは有効event 512件以下に固定し、上限超過payload、facility不一致、range外stay date、不正timestamp / room ID / rank型、aborted / stale generation、invalid root、401 / 403 / 429ではwrite 0とする。
- coverage recordは「そのrangeのresponseをruntime validationできた」ことだけを表す。event 0件、特定stay dateやroom groupのevent欠損を、`rank変更なし`、据え置き、complete historyへ読み替えない。no-change controlは、endpointのpagination / retention / filterを含む完全性、日次current rank snapshot、前後同rank、評価窓内eventなし、変更前pace / LT / 曜日 / capacityの比較可能性が別decisionで確認されるまでdisabledとする。
- pure coverage modelはrank ladderの高rankから低rankへの配列順でindex差が1のeventだけを隣接1段変更として採用する。同じfacility / room group / reflected date / directionで連続するstay dateを1 adjustment episodeへまとめる。同じ日に複数roomまたは複数stay dateを調整した場合、stay date member数やepisode数を独立事例数とせず、facility + reflected dateを同じdecision clusterとして数える。
...
- 隣接 rank が存在しない理由が rank ladder の端である場合、workspace の task または選択詳細には `上限ランク: 上げ余地なし` または `下限ランク: 下げ余地なし` を表示する。rank ladder が取れない、または current rank code が ladder に存在しない場合は、原因を diagnostics に残し、従来どおり `上げ検討` / `下げ注意` の direction 表示に戻す。
...
#### Next Decision Workspace Design Evidence (Unpublished, 2026-07-20)

以下は Next の未公開設計証拠であり、Classic の現公開 layout 契約ではない。既存カレンダーを空間的な基準として残し、`今日の判断` rail と選択中候補の詳細を組み合わせる案を記録する。表示中の 3 か月それぞれに読み取り可能な幅を残せない標準 desktop では、カレンダーを全幅のまま先に置き、rail と詳細をその後ろへ縦に積む。カレンダー 1 か月あたり 560px、rail 最大 356px、workspace gap 14px を同時に確保できる十分な横幅だけで横 2 列を許容する。Revenue Assistant の期間切替、表示切替、標準操作領域を押し下げたり、標準要素へ global style を当てたりしない。

Next の現在の主仮説は `基準日レンズ` とする。カレンダー全体を常時 cluster 色分けせず、利用者が 1 日を基準日に選んだ後だけ類似日を浮かせる。合成 fixture では最大 6 日までとし、基準日変更時は比較対象を reset する。類似度は OH、個人 pace、団体 pace、競合価格を別 dimension として計算・説明し、画面でも `OH`、`個人`、`団体`、`競合` を統合ラベルへ潰さない。欠損 dimension は明示し、個人とほか 2 dimension 以上が揃わない日を強い類似候補にしない。同曜日は説明 tag に使えるが、初期 scoring の隠れた加点にはしない。類似日を選択しても rank 変更、一括調整、write API を自動実行しない。重み、閾値、最大件数、部分比較の操作は合成 fixture の検証値であり、実データ分布と業務判断で評価するまで production contract にしない。

合成 model では個人 pace の一致度0.6以上を必須とし、比較できる根拠を4軸中3軸以上に限定する。総合一致度と根拠充足度は分けて表示し、全4軸が揃わない候補を最上位 tier にしない。booking pace の観測点は配列 index ではなく `leadDays` で整列する。複数日の mini curve は全 card で同じ縦尺度を使い、0を視認用の正値へ持ち上げない。これらの数値自体は production threshold ではなく、誤読を防ぐ fixture contract である。

Next単独運用では、基準日レンズのbooking curve coverageをClassic cacheの偶然の残存へ依存させない。`RAU-UX-151`のNext専用bootstrap / daily deltaが、表示中stay dateのホテル全体と全room group current sourceを段階補充する。選択中基準日のcurrent exactを先に揃え、ほかの日も保存済みの過去pointは期限切れにせず、前回source as-ofより後の不足tailだけを補う。取得中または本日tailのcoverage不足の間は`類似日なし`と断定せず、`比較準備中`と比較可能日数を表示する。候補表示はcurrent tailまで比較可能な日だけから作り、missing / tail未補充 / facility・room group不一致を0としてscoreへ入れない。background sourceが増えた場合は選択を維持したまま比較を再計算し、最大件数やscoring thresholdを取得進捗に応じて緩めない。

`0日前`と`ACT`は類似日レンズでも同一視しない。宿泊日後に初めて取得した着地を過去の`0日前`へ戻さず、分離保存開始前で真の`0日前`を確認できないsourceは`0日前`欠損のまま扱う。直近型referenceの`ACT`は宿泊日後のlanding証拠だけを使い、同じrooms値を`0日前`と`ACT`の両方へ推測配置しない。`0日前`欠損を`1日前`とACTから表示補間することもしない。

初回bootstrapは表示中日付のcurrent sourceとホテル全体の直近型referenceを対象にする。room group referenceはAnalyzeでそのscopeを選んだときだけ不足分を補う。独立した±7 / ±14日同曜日補助線、季節型reference用source、週 / 月 / 隣接日prefetchは基準日レンズのbootstrapへ含めない。取得中も標準calendar、通常日付click、Analyze導線、標準booking curveを隠したり置き換えたりしない。

以下の rail / detail 契約は、基準日レンズと比較する prior design evidence として残す。Next の最終 layout として固定せず、同じ情報や操作を基準日レンズ上でより少ない画面遷移で扱える場合は置き換えてよい。
...
- 見出しに宿泊日と roomGroup、判断方向を表示し、現在 rank と候補 rank を対で表示する。
- 在庫指標は `OH n / キャパ m`、需要内訳は `個人 n` と `団体 n` を別項目で表示する。OH / キャパは current settings、個人 / 団体はそれぞれの直接取得値を使う。別項目との差し引きでは作らない。
- OH、キャパ、個人、団体を取得できない場合は、その項目を `未取得` とする。0 は有効値として `0` と表示する。
- 選択中の 1 候補だけについて、booking curve の全体 / 個人 / 団体、reference、主要根拠、注意、前回変更、更新状態を表示または hydrate する。全候補分の重い evidence を同時に描画しない。
- 深い確認が必要な場合は `Analyzeで確認` を使う。`様子見` と `対応不要` は browser-local decision として維持する。
- rank 変更の初期操作は `変更内容を確認` とし、task card、カレンダー、初期詳細に最終送信 button を置かない。最終確認 region 内だけに `この内容で変更する` と `確認をやめる` を置き、前者の明示押下だけが送信前 guard を開始できる。確認を開いたまま待っても送信しない。
- warm cache の月別優先取得と状態表示は、workspace の主判断導線を分断しないよう選択詳細の後へ置く。取得中、完了、skip、失敗、停止、retry 待ち、ログイン確認、権限確認を画面内の文脈で確認できるようにする。

first wave の workspace では、推奨レート金額、forecast 数値、sales / ADR 数値、競合価格の金額、差額、percent を task card や判断 summary の主表示へ追加しない。これらを表示候補に戻す場合は、入力データ、比較対象、判断条件、誤読を避ける表示単位を別 task で先に決める。

#### Classic Published Top List Contract (Frozen)
...
- `競合価格` preview。料金調整候補 row から競合価格を確認する導線は、常時表示の列を増やさず、`曲線` と同じ row 直下 preview として押下時だけ開く。button は、対象 `stayDate` があり、競合価格 snapshot adapter が使える top row で表示する。開閉、`Escape` close、focus return は `曲線` preview と同じ契約にする。preview の対象日は row の `stayDate` とし、対象 room type は候補の `roomGroupName` と `jalanFacilityRoomType` の対応が確認できる場合だけ強く絞る。対応確度は `confirmed`、`ambiguous`、`unknown` の 3 段階で扱う。`confirmed` は、保存済み snapshot の部屋タイプ label が `roomGroupName` に一意に含まれる場合だけとし、その label で preview graph を絞り込む。`ambiguous` は複数 label が候補になる場合、`unknown` は label が見つからない、または snapshot 側に比較できる label がない場合とする。`ambiguous` / `unknown` では、保存済み snapshot 全体の人数別最安値 graph と状態文を表示し、部屋タイプ対応未確認を金額推奨や候補方向の主因として扱わない。preview の読み順は、state message、部屋タイプ対応 note、graph または empty / error state の順にする。部屋タイプ対応 note は利用者向けには日本語優先で表示し、DOM には `data-ra-rank-recommendation-competitor-preview-room-type-note` marker を付ける。cache hit では保存済み snapshot を表示し、未取得または不足時は押下後に対象日の競合価格 snapshot 取得を開始する。同じ `facility x stayDate` の in-flight request は重複発行しない。取得中、取得失敗、データなし、再取得を preview 内に表示し、金額、差額、percent を top list 本文や summary へ常時表示しない。Revenue Assistant write API、rank 変更 POST、自動反映、一括反映は追加しない。
...
- decision workspace の `今日の判断` rail、target month、work-state control、task card、選択詳細、最終確認 region。
- workspace 内の read-only 表示部品。例: 現在 / 候補 rank、OH / キャパ、個人 / 団体、主要根拠、booking curve evidence host。
- workspace 内の browser-local UI state。例: 選択中 task、work state、表示件数、target month、最終確認 open state、rank select state。
- workspace 内の pending / result 表示。例: `様子見`、`対応不要`、rank 変更の反映確認中、成功、未確認、失敗。
...
- 入力は、`RankRecommendationReactListSnapshot` と、その中の candidate、button、control、evidence placeholder snapshot に限定する。
- `derived from snapshot` は、task card、表示文言、button title、disabled 状態、evidence placeholder key である。work-state 判定、OH / キャパ、個人 / 団体、reason summary を component 内で再計算しない。
- `browser-local UI state` は、React component 内で持ってよい。対象は選択中 candidate key、最終確認 open state、選択中 rank code のように、表示と送信候補だけを変える一時 state に限定する。
- rank change の active / confirming / result state は main module が管理し、React component は snapshot に含まれる状態を表示する。React component は `この内容で変更する` の delegated action を表すだけで、API adapter を直接呼ばない。
- `temporary display state` は、task selection、最終確認 open state、focus state である。Revenue Assistant API request、IndexedDB write、rank change POST は component 内で直接行わない。
- 出力は scoped `data-ra-*` selector、button attribute、link href、aria attribute、evidence placeholder DOM とする。workspace prior design の smoke selector は設計証拠として維持し、Next 基準日レンズの合格条件とは分ける。
...
- write API に近い操作へ primitive を適用する場合は、二段階の明示確認、送信前取消、送信直前 current rank 再取得、rank status 再取得、反映確認、同一 `stayDate x roomGroup` active change block を維持する。確認を開いた状態や経過時間だけで POST しない。

2026-07-17 workspace overhaul prior design evidence (unpublished):
...
- `data-ra-rank-recommendation-ui-component` は `workspace-rail`、rail header / controls / task list、detail、review、pending、result の確認入口として使う。この prior design の確認では Classic table / row marker を条件にしないが、Classic 公開 smoke の合格条件を変更しない。
...
`様子見` と `対応不要` は、押下直後に確定しない。5 秒の in-memory pending state に入り、選択詳細に残り秒数 text、progress ring、`取消` button を表示する。progress ring は text と同じ `commitAt` と 5 秒 duration から計算し、表示補助だけに使う。progress ring は pending 秒数、保存条件、decision lifecycle を変更しない。
...
rank 変更は 5 秒 timer で送信しない。`変更内容を確認` で最終確認 region を開き、`この内容で変更する` の明示押下後にだけ送信前 guard へ進む。送信前 guard、POST、反映確認のいずれかに失敗した場合、対象候補を隠したままにせず、同じ選択詳細または次の再同期後の task で失敗状態を確認できるようにする。送信成功または反映確認済みの場合は、既存の resolved rank change filter により対象候補を非表示にする。

## Current Rank Occupancy Display

トップ decision workspace の選択詳細では、current rank と在庫内訳を分けて表示する。在庫は `OH current / キャパ max` とし、OH は `/api/v1/suggest/output/current_settings` の `max_num_room - remaining_num_room` で計算する現在の使用済みまたは確保済み室数、キャパは同じ response の `max_num_room` を指す。

個人と団体は、booking curve の `transient` と `group` からそれぞれ直接取得した現在値を `個人 n`、`団体 n` として別表示する。`all - group`、`OH - 団体`、`キャパ - remaining` などの差し引きで個人または団体を推測しない。`remaining_num_room`、`max_num_room`、`transient`、`group` の各値が未取得または不正な場合は、該当項目を `未取得` とする。0 は有効値として表示する。これらは UI 表示用であり、candidate scoring、priority、confidence、reasonFingerprint、rank change payload には使わない。

トップカレンダーの標準画面由来の黒い数値は、OH と同じ source / semantic であることを確認できていないため relabel しない。カレンダーでは既存の黒い数値と青い `団n` を維持し、OH / キャパ、個人、団体の明示は選択詳細で行う。
...
- user decision、cooldown、dismissed、resolved の browser-local lifecycle は first phase の decision workspace で維持する。ただし、bulk apply 用の preview、複数選択状態、反映結果保存、部分失敗保存は未実装である。
...
10. 競合価格内自社料金位置を roomGroup 別に強める場合、Revenue Assistant の roomGroup と `jalanFacilityRoomType` をどの確認済み source で対応づけるか。`RAU-CP-23` では top preview 表示上の `confirmed` / `ambiguous` / `unknown` は導入したが、これは表示絞り込み用の保守的な分類であり、候補方向や confidence 補正へ使う source にはしない。
