...
20件はSLO complianceを主張するための母数であり、計画due task集合とsession capを変えない可逆なpace試験の開始条件ではない。`RAU-PERF-21B`は`RAU-PERF-21`を100ms / 30で公開・検証した同じsource revisionのcontrolで、通常利用由来のNext booking curve GETを合計50件以上観測し、安全gateを通した後に50ms / 20を別request profileとして試せる。controlとcandidateのsampleは同じp95母集団へ混ぜず、source revision、request profile、room数band、warm / revalidateを分けて比較する。
