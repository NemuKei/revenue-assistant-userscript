...
`warm`は必要なexact current sourceが操作開始時にNext storeまたは有効なClassic read-through seedへ存在する状態、`revalidate`は必要なcurrent sourceの一部が未保存またはdueである状態を指す。施設全体の空storeから行うbounded bootstrapはbackground準備であり、interactive SLOの完了条件へ含めない。background完了時間にはSLOを置かず、TopとAnalyzeが同じsession上限を共有する場合も、Top backgroundが将来のAnalyzeで必要なbounded headroomを使い切ってはならない。優先順は、Analyze overall current、利用者が開いたroomのcurrent、同roomのreference / rank marker、1〜12 room summary、Topの画面外backgroundとし、選択roomがない間は次に利用可能なforegroundへ枠を使う。interactive pending中は同じcontextの新しいbackground requestを開始せず、priorityを上げただけでは同じcontextで開始済みのrequestをpreemptしない。最初のforeground保護は既存bootstrap 800 / daily 200内のreserve再配分とし、未使用枠はbackgroundへ返す。総上限、scope、long-session tokenの増加は含めない。route / facility / as-of / visible-range context変更、document hidden、runtime停止では既存契約どおりqueued / in-flightをabortできる。
