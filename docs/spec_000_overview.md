...
- トップ画面では、カレンダー badge だけに頼らず、料金調整候補リストを優先度順に表示する first wave を候補にする。
...
- トップ料金調整候補 list の summary、controls、table、row、cell、row actions、preview row、pending 表示、booking curve 要点 popover。
...
6. production 接続はトップ料金調整候補 list の 1 component から始め、既存 `data-ra-*` selector、pending、preview、write guard を維持する。
...
- `main` push では、`.github/workflows/publish-userscript.yml` の path filter を使い、userscript 実行版または配布検証に影響するファイルだけを Publish Userscript workflow の起動対象にする。
- 起動対象は、`src/**`、`scripts/**`、`dev/fixtures/**`、`package.json`、`package-lock.json`、`userscript.config.mjs`、`vite.*.config.mjs`、`tsconfig.json`、`eslint.config.mjs`、workflow 自体である。
- `docs/**`、`README.md`、`AGENTS.md` だけの push は、userscript 実行版が変わらないため Publish Userscript workflow を起動しない。これにより、docs-only closeout だけで GitHub Pages 公開版 version が進む状態を避ける。
...
- `docs/spec_003_rank_recommendation_signal.md`: トップ料金調整候補リスト、推奨ランク方向、user decision、candidate lifecycle、rank response、future bulk apply の契約
...
- トップ料金調整候補リスト、推奨ランク方向、user decision、candidate lifecycle、rank response、future bulk apply は `docs/spec_003_rank_recommendation_signal.md` を正本とする。
