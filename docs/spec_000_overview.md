...
- 月次実績画面の日次差分 compact view の確認用 fixture state。
- 価格推移 supplement の loading、empty、failure 確認用 fixture state。
...
3. fixture へ主要 state を追加し、実データ、Cookie、token、価格や在庫の非公開データを使わずに確認する。
...
- 対象 scope では、主要 selector、React marker、console / page error 0 件、監視対象 write API POST 0 件を確認する。
