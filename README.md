...
- 現在のNext公開版`0.2.0.39`は、Top background 50ms / concurrency 20、Analyze foreground 35ms / concurrency 20、Top 800 / 200、scope別coalesced refreshを維持しつつ、Analyzeのroom差分更新をdirty scope単位へ狭め、保存済みbooking curveの重複model構築を削減した。booking curve、90日価格推移、競合価格では、独自Tooltipと重なるbrowser native Tooltipを発生させるSVG `title`を持たず、読み上げ名を`aria-label`、説明を`desc` + `aria-describedby`で維持する。booking curveの横軸は全LTのTooltip / 表を保ったまま、表示labelを非重複の節目へ整理した。取得対象、due / dedupe、endpoint、storage / retention、retry / stop、Revenue Assistant write 0は変更していない。20 sample未満のSLOは引き続きprovisionalとする。
...
現在の公開Nextはversion `0.2.0.41`、source `73d45d94e706bdbd0107e2aaf72481bab584f42b`、manual workflow run `31856977602`です。userscriptは374,147 bytes、SHA-256 `479FD93F44D6DECAEA2CE4EB30EA7C002FF159145770893B614158F39E3D96B4`、source mapは1,502,141 bytes、SHA-256 `86640B9A799707C94D97992CDE93A3F490EB727AF53656D0173638B96FEDDE1D`でrelease manifestと一致し、Classic userscript / source mapは固定baselineと同一byte列です。
