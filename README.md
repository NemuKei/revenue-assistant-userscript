...
- 現在のNext公開版`0.2.0.44`は、Topの`基準日から似た日を探す`を標準カレンダー全体の直後へ下げ、日常的に見るカレンダーを先に表示する。検索機能、選択mode、候補・比較の初期折りたたみ、calendar markerは維持する。Top background 50ms / concurrency 20、Analyze foreground 35ms / concurrency 20、Top 800 / 200、保存済み表示先行も不変である。90日価格推移の部屋タイプ / 食事filter、競合価格と価格推移Tooltipの共通正負色、Analyze販売設定のroom scope順ランク変更履歴を維持し、取得対象、endpoint、storage / retention、retry / stop、Revenue Assistant write 0は変更していない。20 sample未満のSLOは引き続きprovisionalとする。
