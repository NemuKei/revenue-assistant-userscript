...
- 現在のNext公開版`0.2.0.30`は、Top backgroundの800 / 200とAnalyze foregroundを分離した上で、Topは団体cueに必要な可視hotel currentを全日分先に取得し、有限foregroundだけを35ms / concurrency 30へ緩和する。Analyzeは保存されたscopeだけを再読込し、250ms内の通知を1回の再描画へまとめる。Top backgroundの50ms / concurrency 20、取得対象、due / dedupe、endpoint、storage / retention、retry / stop、Revenue Assistant write 0は変更していない。保存済みsourceは先に表示し、cold scopeは`比較準備中`のまま完了scopeから描画する。自然revalidateと20 sample SLOは通常利用で観測を続ける。
