...
現時点の `src/main.ts` は、レベニューアシスタントの analyze 日付ページを検知し、月次カレンダーの各日付セルへ団体室数を追記する PoC までを実装しています。ブッキングカーブ API の結果は、画面上の `最終データ更新` 日付が変わるまで localStorage にキャッシュします。
