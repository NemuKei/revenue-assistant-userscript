...
- 現在のNext公開版`0.2.0.24`は、部屋タイプ別booking curveのrank変更Tooltipへ、既存rank status responseで確認できる変更者をClassicに合わせて戻した。optional stringだけをpage-memoryのTooltip / aria labelへ渡し、履歴表、取得回数、storage、rank-learning allowlist、Revenue Assistant write 0は変更していない。source `54bd92665d42f1ed4876304a8a957c6e7ce2590d`、manual workflow run `31664684495`のbuild / deploy / verify、公開artifact / Classic baseline照合まで完了した。通常Chromeのinstalled runtimeはまだ`0.2.0.23`で、Tampermonkey更新と再ログイン後の実画面受け入れを残す。
