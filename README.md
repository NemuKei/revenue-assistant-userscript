...
- 現在のNext公開版`0.2.0.23`は、standalone / embeddedの`調整後のペース`blockとevent別pace評価を廃止し、振り返りをbooking curveのrank marker、通常Tooltip、折りたたみrank履歴へ集約した。block専用rank sequences readも外し、既存chart、rank status、`RAU-RR-66`のsanitized learning DBを維持する。source `d2b0633d01235df0c496aee29c2ca36c80d48659`、manual workflow run `31655859440`のbuild / deploy / verify、公開artifact / Classic baseline照合、Tampermonkey更新後の通常Chrome受け入れまで完了した。installed runtime `0.2.0.23`で販売設定2 room、標準booking curveのroom scope、販売設定への復帰を確認し、対象block 0、rank sequences追加GET 0、Revenue Assistant write 0だった。
