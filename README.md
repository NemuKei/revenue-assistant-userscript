...
- 現在のNext公開版`0.2.0.30`は、Top backgroundの800 / 200とAnalyze foregroundを分離した上で、Topは団体cueに必要な可視hotel currentを全日分先に取得し、有限foregroundだけを35ms / concurrency 30へ緩和した。Analyzeは保存されたscopeだけを再読込し、250ms内の通知を1回の再描画へまとめる。ホテル関西liveでは35ms paceとGET / HTTP 200 / write 0を確認した一方、最大同時30の連続取得中に数秒の操作応答悪化を再現したため、次のsource補正は開始間隔35msを維持してforeground concurrencyだけ20へ戻す。Top backgroundの50ms / concurrency 20、取得対象、due / dedupe、endpoint、storage / retention、retry / stop、Revenue Assistant write 0は変更しない。保存済みsourceは先に表示し、cold scopeは`比較準備中`のまま完了scopeから描画する。自然revalidateと20 sample SLOは通常利用で観測を続ける。
...
2026-08-14にsource `a54aeace43cc9541405a236481186ade0b1d1a61`をmanual workflow run `31773118598`で公開版`0.2.0.30`へ配信しました。公開Nextは367,869 bytes、SHA-256 `9D3CCE850B66DE52D0840A27FECB6159E4C87FBB1EBBBB71447602B47A4D0FF9`、source mapは1,458,844 bytes、SHA-256 `415B6CF0FB162D2B953553F6C33A79C11BDCA33250F8DC47A0ED0C6486733DAB`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscript / source mapは固定baselineと同一byte列です。Tampermonkey更新後の通常Chrome post-change sampleは未取得です。
