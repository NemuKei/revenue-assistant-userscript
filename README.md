...
ただしこのworkflow分離はまだlocalだけです。2026-07-22のfresh fetch時点でlocal mainはorigin/mainより11 commits ahead / 6 commits behindで、remote側の依存 / Actions更新は明示承認なしに取り込みません。GitHub Actionsはpushに対応するcommit上のworkflowでtriggerを判定するため、承認後はorigin更新とverify-only workflowを同じ最終treeへ統合し、1回のpushで反映します。依存更新だけを先にpushしません。push後にClassic Publish run / Pages deploymentが発生していないことと公開SHA不変を確認するまで分離完了とはしません。Nextの公開、Tampermonkey install、Classicからの切替は、それぞれ別gateです。
