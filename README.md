...
- 現在のNext公開版`0.2.0.27`は、`0.2.0.26`の5段階queueと未選択room reference遅延を維持しつつ、booking curve開始間隔を100msから50ms、concurrency上限を30から20へ変更した可逆な第一段階candidateである。bootstrap 800 / daily 200、interactive reserve 32、取得対象、due判定、保存、停止条件は変えていない。別の自施設の自然なcold windowではbackground 768件を約133秒で完了し、最大同時20、外部timestamp最短51ms、candidate error / 401 / 403 / 429 / Revenue Assistant write 0だったため、50ms profileの負荷安全sub-gateは通過した。一方、同じsessionでAnalyzeがreserve 32件を使い切った後は選択room currentが23msでreadyでもreference根拠が20秒以内にsettleしなかった。利用者判断により、次は35ms化や総request増ではなく、既存800 / 200内でTop backgroundからAnalyze foregroundへ枠を再配分する`RAU-PERF-22`を優先する。runtimeと公開版はまだ変更していない。
- `RAU-PERF-22`のsource candidateでは、利用者の明示承認に基づきTop backgroundの800 / 200とAnalyze foregroundを分離した。AnalyzeはTop上限で停止せず、現在のhotel / room currentと開いたroomの有限reference due taskを50ms / concurrency 20、dedupe、停止条件付きで取得する。Analyze routeではbackground plannerを開始せず、hotel描画後の全room currentを並列queueへ入れてscopeごとにprogressive描画する。performance profileは`booking-curve-50ms-20-analyze-uncapped`へ分けた。保存、retention、endpoint、Revenue Assistant write 0は変更していない。focused / Next全check、実ブラウザfixture、candidate / synthetic publication build、Classic boundary、distribution / booking curve smoke fixtureは通過し、公開と通常Chrome SLO確認はこれから行う。
...
現在の公開Nextはversion `0.2.0.27`、source `8d9ff5469920005565ced6e9e359160f2627f87b`、manual workflow run `31679952896`です。userscriptは364,958 bytes、SHA-256 `8D9AC63CCA29F1E8B03EDE5D9F72FF1F8D4818CBFDE19250CE365D5994BF2D59`、source mapは1,444,939 bytes、SHA-256 `50A98FB2ECDC52E0ECD7546ED833EC5A187920D462B81E6FCAEDC1D8A36E6F78`でrelease manifestと一致し、Classic userscript / source mapは固定baselineと同一byte列です。次のsource candidateはTop / Analyze budget分離とAnalyze room current並列化のlocal gateを通過し、manual publication待ちです。
