...
- 現在のNext公開版`0.2.0.26`は、`0.2.0.25`のprivacy-safe performance marker / collectorを維持しつつ、booking curve取得を`critical-current`、`visible-current`、`selected-reference`、`visible-reference`、`background`の5段階queueへ分けた。販売設定ではhotel / 全room currentをreferenceより先にし、未選択room referenceはcard openまで遅延する。source `c324986ed229c82744b5ce146c6aded6cef7d6a1`、manual workflow run `31676935570`のbuild / deploy / verify、公開artifact / Classic baseline照合、Tampermonkey更新、通常Chromeの100ms / concurrency 30 controlまで完了した。通常操作でbooking curve start 68件、最大同時16、error / 401 / 403 / 429 / write 0を確認し、次の50ms / concurrency 20候補をsourceで検証済みである。SLO cohortは20 sample未満のためprovisionalを維持する。
...
現在の公開Nextはversion `0.2.0.26`、source `c324986ed229c82744b5ce146c6aded6cef7d6a1`、manual workflow run `31676935570`です。userscriptは364,960 bytes、SHA-256 `300FEE3E78FA5C3180B73DDD612B26CD1F75AE597AF32436D7398E8F79BE65B8`、source mapは1,444,941 bytes、SHA-256 `E7ACAAFF8592A1E8C0769363CD5021A490CCB36DB4470E03AD4FCE8D6ED08857`でrelease manifestと一致し、Classic userscript / source mapは固定baselineと同一byte列です。Tampermonkey更新と通常Chromeの100ms / concurrency 30 controlまで完了しています。
