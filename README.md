...
- 現在のNext公開版`0.2.0.29`は、Top backgroundの800 / 200とAnalyze foregroundを分離した。AnalyzeはTop上限で停止せず、現在のhotel / room currentと開いたroomの有限reference due taskを50ms / concurrency 20、dedupe、停止条件付きで取得する。Analyze routeではbackground plannerを開始せず、hotel描画後の全room currentを並列queueへ入れてscopeごとにprogressive描画する。同一facility / stay date / scopeの有効な保存済みsourceがある場合は、`最新データを更新中（保存済みデータを表示）`を示してcurve / summaryを先に描画し、fresh currentを同じ有限queueで非同期に取得する。保存済みsourceがないcold scopeは`比較準備中`を維持し、完了scopeから描画する。stale表示はfresh milestoneへ数えない。request数、endpoint、50ms / 20、storage / retention、retry / stop、Revenue Assistant write 0は変更していない。ホテル関西warmでoverall / 6 room summary各8ms、選択room current 19msを確認した。自然revalidateと20 sample SLOは通常利用で観測を続ける。
