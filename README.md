...
- 現在のNext公開版`0.2.0.43`は、Top background 50ms / concurrency 20、Analyze foreground 35ms / concurrency 20、Top 800 / 200、保存済み表示先行を維持する。90日価格推移には競合価格と共通の部屋タイプ / 食事filterがあり、利用者が選んだ1部屋タイプの不足scopeだけを有限取得する。競合価格と価格推移のTooltipは、共通moduleでプラスを緑、マイナスを赤、0・欠損を中立へ揃え、`+ / -`も残す。Analyze販売設定のランク変更履歴は、各roomの最新eventを保ったまま標準の室タイプカードと同じ順に並べる。取得対象、endpoint、storage / retention、retry / stop、Revenue Assistant write 0は変更していない。20 sample未満のSLOは引き続きprovisionalとする。

現在のNext導入リンク: [Revenue Assistant NextをTampermonkeyでインストール / 更新](https://nemukei.github.io/revenue-assistant-userscript/next/revenue-assistant-next.user.js)
- `RAU-RR-66`のTop calendar live gateも完了した。通常Chromeの次月表示 / 今月復帰でrank status各1 GET、rank sequences 0、Revenue Assistant write 0を確認し、値を出さないreadonly probeでrank-learning DBのschema、件数、field allowlistを照合した。実分布は`RAU-RR-67`の停止条件に達していないため、上下判断UIはまだ実装しない。詳細なlive snapshotは`docs/context/STATUS.md`を正とする。
...
npm run smoke:distribution -- --installed-version <Tampermonkey上のversion> --mode competitor-prices --url https://ra.jalan.net/analyze/YYYY-MM-DD --seconds 45
...
現在の公開Nextはversion `0.2.0.43`、source `1ac3f8244a3632857ae9b45b77b75689ba7501b1`、manual workflow run `31874152258`です。userscriptは373,944 bytes、SHA-256 `66C244F8AF6AADD46B6D924A16A2E95F57AD5537E3AC77DC738805BCA9944243`、source mapは1,502,591 bytes、SHA-256 `2A6534FF92DD03080347AE29EDA17D953428E14083BE9EDD33DAB5C4601F963F`でrelease manifestと一致し、Classic userscript / source mapは固定baselineと同一byte列です。
...
2026-08-10にsource `85061f1cfec7b736f4bccc3a35bf5348901ba450`をmanual workflow run `31356748421`で公開版`0.2.0.9`へ配信しました。公開Nextは276,840 bytes、SHA-256 `DD78D2DFCE66EC0445A2D82CC24C972C339B325A4E4CD41A8A4AC592E7E82153`でrelease manifestと一致し、Classic userscript / source mapは固定baselineと同一byte列です。

2026-08-10にsource `d683b9c558c167748aadde86d73cad69c9f96d78`をmanual workflow run `31358578449`で公開版`0.2.0.10`へ配信しました。公開Nextは280,281 bytes、SHA-256 `7BDAC1D59109D63527A02AFDF7400BEBD40608E3E14DAEDC27E599032AAA3F64`でrelease manifestと一致し、Classic userscript / source mapは固定baselineと同一byte列です。
...
2026-08-12にsource `c88a5bb663fd613effd0b43854ba0a74d13a10b5`をmanual workflow run `31566094703`で公開版`0.2.0.20`へ配信しました。公開Nextは353,192 bytes、SHA-256 `20BCF941C10304CA431F09CBADB9A2CE104D9409F204A8B9B58EC3648FFBEB0E`、source mapは1,360,010 bytes、SHA-256 `756D1DF26676E21B7F245E064931AF1529CBF00CDB0A073AE76E3834CFFBA271`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscript / source mapは固定baselineと同一byte列です。Tampermonkey更新と、通常Chromeの販売設定から標準booking curveへのtab切替を含むrequest budget / write 0確認は公開後の別確認です。

2026-08-12にsource `fed091d3ea4e793d08a9f294c12c7c694cd17ade`をmanual workflow run `31568825506`で公開版`0.2.0.21`へ配信しました。公開Nextは353,544 bytes、SHA-256 `6D6D2239E19DA8069FC5DBB3D84BB772E02F1BAC232E278233A94E53432D1EB9`、source mapは1,362,728 bytes、SHA-256 `A1C6327A86A2E2A4B1830DDA91F0662AFC9ADC3A3C2E48AE818FC85AAAD3B9C2`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscript / source mapは固定baselineと同一byte列です。Tampermonkey更新後の通常Chromeではinstalled runtime `0.2.0.21`を確認しました。販売設定で2 roomを開いた状態から標準booking curveのroom scopeへ移り、販売設定へ戻る安定した監視区間はtruncatedなしで、Revenue Assistant request 30件はすべてGET、失敗0件でした。Next由来のrank status / rank sequencesは各1回で、復帰時の追加rank読取は0件、Revenue Assistant writeは0、console warning / errorは0、標準の反映button 11件は非干渉でした。Chrome拡張のmouse入力が人工的なhidden transitionを起こした区間は棄却し、同じ通常Chrome tabの正確なDOM操作を使った区間だけを合格根拠にしています。

2026-08-13にsource `d2b0633d01235df0c496aee29c2ca36c80d48659`をmanual workflow run `31655859440`で公開版`0.2.0.23`へ配信しました。公開Nextは348,207 bytes、SHA-256 `23289D3C50B4CCE25BD2581C34A14F780E15DDDE334E8051C76DBF17A84C03CA`、source mapは1,350,167 bytes、SHA-256 `C10B4B7D04B3A5986B549813E9C1F308C0C477CA9A649136395E155A4913A24E`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscriptはversion `0.1.0.442`、662,626 bytes、SHA-256 `6C4635639376A6ECA2259FC9EA7916141CFE1A40BD3AE1364E49F577030802EB`の固定baselineと同一byte列です。利用者のTampermonkey更新後、通常Chromeでinstalled runtime `0.2.0.23`を確認しました。販売設定で2 roomを開き、標準booking curveのroom scopeへ移り、販売設定へ戻る安定した各監視区間はevent欠落なしで、対象blockとrank sequences追加GET、Revenue Assistant write、HTTP失敗、runtime例外はいずれも0でした。販売設定復帰時はembedded component 3、panel 6、rank marker 26、ready rank history 2、標準booking curveのroom scopeはroot 1、panel 2、rank marker 14、ready rank history 1で、各Next rootのoverflowは0でした。

2026-08-13にsource `54bd92665d42f1ed4876304a8a957c6e7ce2590d`をmanual workflow run `31664684495`で公開版`0.2.0.24`へ配信しました。公開Nextは348,624 bytes、SHA-256 `A8DFA5720F4EEEF8AB2CA9EFD052CECD7C383F227D006A471B61427F0EB8EDFA`、source mapは1,351,480 bytes、SHA-256 `A26926C3D941534554CB1AFFE24425B0403899FA1AF12A2ACCA77CA6E3DF2077`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscriptはversion `0.1.0.442`、662,626 bytes、SHA-256 `6C4635639376A6ECA2259FC9EA7916141CFE1A40BD3AE1364E49F577030802EB`の固定baselineと同一byte列です。通常Chromeではinstalled runtime `0.2.0.23`と再ログイン案内を確認したため、Tampermonkey更新とログイン後の部屋タイプ別Tooltip受け入れは未実施です。

2026-08-13にsource `adeff29a2aec6172d34dc839ac1c3a411d61a835`をmanual workflow run `31672723475`で公開版`0.2.0.25`へ配信しました。公開Nextは363,259 bytes、SHA-256 `5101063E32E9C6C8BEE2C3D4E41A4842AA2E0BDDA91A3C666BB6DBDF8DF2E3EC`、source mapは1,436,673 bytes、SHA-256 `CC5CB9290EF49CE4D228164FF6340A9180A0C5482E6F4268EF5BB4A0BADFDFFA`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscriptはversion `0.1.0.442`、662,626 bytes、SHA-256 `6C4635639376A6ECA2259FC9EA7916141CFE1A40BD3AE1364E49F577030802EB`の固定baselineと同一byte列です。Tampermonkey更新後の通常Chromeでinstalled runtime `0.2.0.25`、marker 1件、schema `rau-next-performance-v1`、source revision一致を確認しました。初回live baselineはTop、Analyze、選択room、競合価格の各1 sampleだけなのでprovisionalです。warm Analyzeは全体8,327ms、全room 14,837msで、`RAU-PERF-21`のcurrent優先 / reference head-of-line解消へ進むdescriptive evidenceとしました。基準日判断は自動操作待ちを含んだsampleを除外し、自然なrevalidateと各cohort 20 sampleは継続観測します。

2026-08-14にsource `a54aeace43cc9541405a236481186ade0b1d1a61`をmanual workflow run `31773118598`で公開版`0.2.0.30`へ配信しました。公開Nextは367,869 bytes、SHA-256 `9D3CCE850B66DE52D0840A27FECB6159E4C87FBB1EBBBB71447602B47A4D0FF9`、source mapは1,458,844 bytes、SHA-256 `415B6CF0FB162D2B953553F6C33A79C11BDCA33250F8DC47A0ED0C6486733DAB`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscript / source mapは固定baselineと同一byte列です。Tampermonkey更新後のホテル関西liveで35ms pace、GET / HTTP 200 / write 0と、foreground最大同時30での数秒の操作応答悪化を確認しました。

同日にsource `d6d9ef203412be3d274dae6b47074320cbdd946d`をmanual workflow run `31774630650`で公開版`0.2.0.31`へ配信しました。公開Nextは367,856 bytes、SHA-256 `3CD3171DFBC41CBB2D01EAAA71933F6EB49716A555CBCFDE87F2E6F6B6E5B797`、source mapは1,458,811 bytes、SHA-256 `752EDC2B7BCCE95C31BB44EB4948F956D9C337D998CC922275F517859EC7C12B`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscript / source mapは固定baselineと同一byte列です。foregroundは35ms / concurrency 20で、Tampermonkey更新後の通常Chrome再確認を残します。

同日にsource `5e61e822454c5f65e2ec8d1e16bba057e1e729ce`をmanual workflow run `31779175865`で公開版`0.2.0.32`へ配信しました。公開Nextは368,402 bytes、SHA-256 `800756CB0D50E4CFF18D26DD1EDDE3D1B27803DAF09CF05E954FC4BF5CEFB943`、source mapは1,461,398 bytes、SHA-256 `B342CC7A442F679526CE8C219A9C4C403DA60F506952B093B3D2A9265C7C6CC3`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscript / source mapは固定baselineと同一byte列です。Tampermonkey更新後のホテル関西Analyzeでinstalled runtimeとprofileを照合し、自然な差分64件を持つ2 roomの根拠表示1,488〜1,658ms、最大同時20、scheduler error / stop 0、Long Task 0、軽量DOM probe 28 / 28成功を確認しました。CDP retained windowはtruncatedのため全64件の外部証明には使わず、保持された35 request / 35 responseはすべてGET / HTTP 200、loading failure / Revenue Assistant writeは0でした。初期6 room summary 7,258msの単発観測と20 sample未満のSLOはprovisionalです。

2026-08-15にsource `73d45d94e706bdbd0107e2aaf72481bab584f42b`をmanual workflow run `31856977602`で公開版`0.2.0.41`へ配信しました。公開Nextは374,147 bytes、SHA-256 `479FD93F44D6DECAEA2CE4EB30EA7C002FF159145770893B614158F39E3D96B4`、source mapは1,502,141 bytes、SHA-256 `86640B9A799707C94D97992CDE93A3F490EB727AF53656D0173638B96FEDDE1D`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscript / source mapは固定baselineと同一byte列です。通常Chromeで読み込まれていたNextは`0.2.0.40`だったため、Tampermonkey更新後の90日価格推移における部屋タイプ別取得・絞り込みの実画面確認を残します。

同日にsource `37e7a8129e2fc4ab0d9e967bebf6f32f624ac8ec`をmanual workflow run `31861058291`で公開版`0.2.0.42`へ配信しました。公開Nextは374,050 bytes、SHA-256 `A80312A4448145D8AF10831C8395BB0E86D7E3F44B303A2BED2BF7852DCC92B7`、source mapは1,502,979 bytes、SHA-256 `3837FF2B9DED6386AFE19D97E5257A47882CDF6412E7AD39E9F508EA6C6DF5B7`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscript / source mapは固定baselineと同一byte列です。Tampermonkey更新後の実画面で、90日価格推移の部屋タイプ別取得・絞り込みと、競合価格 / 価格推移Tooltipの正負色をまとめて確認します。

同日にsource `1ac3f8244a3632857ae9b45b77b75689ba7501b1`をmanual workflow run `31874152258`、run number 43、attempt 1で公開版`0.2.0.43`へ配信しました。公開Nextは373,944 bytes、SHA-256 `66C244F8AF6AADD46B6D924A16A2E95F57AD5537E3AC77DC738805BCA9944243`、source mapは1,502,591 bytes、SHA-256 `2A6534FF92DD03080347AE29EDA17D953428E14083BE9EDD33DAB5C4601F963F`でrelease manifestと一致し、build / deploy / verifyはすべてsuccess、Classic userscript / source mapは固定baselineと同一byte列です。Tampermonkey更新後にAnalyze販売設定のランク変更履歴が標準の室タイプカード順であることを実画面確認します。
