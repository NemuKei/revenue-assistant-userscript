...
- 現在のNext公開版`0.2.0.28`は、Top backgroundの800 / 200とAnalyze foregroundを分離した。AnalyzeはTop上限で停止せず、現在のhotel / room currentと開いたroomの有限reference due taskを50ms / concurrency 20、dedupe、停止条件付きで取得する。Analyze routeではbackground plannerを開始せず、hotel描画後の全room currentを並列queueへ入れてscopeごとにprogressive描画する。performance profileは`booking-curve-50ms-20-analyze-uncapped`へ分けた。保存、retention、endpoint、Revenue Assistant write 0は変更していない。ホテル関西の自然なrevalidateではoverall 1,135msだが6 room summary 14,861msとなり、保存済みsourceをnetwork完了後まで読まない順序が残る待ちの主因だった。
- `RAU-PERF-22`の後続source candidateでは、同一facility / stay date / scopeの有効な保存済みsourceがある場合、`最新データを更新中（保存済みデータを表示）`を示してcurve / summaryを先に描画し、fresh currentを同じ有限queueで非同期に取得する。保存済みsourceがないcold scopeは`比較準備中`を維持し、完了scopeから描画する。stale表示はfresh milestoneへ数えない。request数、endpoint、50ms / 20、storage / retention、retry / stopは変更していない。focused / Next全check、実ブラウザfixture、candidate / synthetic publication build、Classic boundary、distribution / booking curve smoke fixtureは通過し、再公開と通常Chrome確認はこれから行う。
