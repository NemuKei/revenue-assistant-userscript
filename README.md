...
- 現在のNext公開版`0.2.0.42`は、Top background 50ms / concurrency 20、Analyze foreground 35ms / concurrency 20、Top 800 / 200、保存済み表示先行を維持する。90日価格推移には競合価格と共通の部屋タイプ / 食事filterがあり、利用者が選んだ1部屋タイプの不足scopeだけを有限取得する。競合価格と価格推移のTooltipは、共通moduleでプラスを緑、マイナスを赤、0・欠損を中立へ揃え、`+ / -`も残す。取得対象、endpoint、storage / retention、retry / stop、Revenue Assistant write 0は変更していない。20 sample未満のSLOは引き続きprovisionalとする。
