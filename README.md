...
- Next の現在の entry は runtime 境界を確認する read-only shell であり、基準日レンズ UI は合成 fixture にだけ存在する。実画面 API / DOM 接続、Tampermonkey install / switch、publish、write 操作は未実施である。
...
- `npm run dev:next:fixture`: Next の基準日レンズ合成 fixture を `http://127.0.0.1:5173/dev/fixtures/similarity-lens/` で preview
- `npm run build:next:fixture`: Next の基準日レンズ合成 fixture を `.tmp/vite-next-fixture/` に生成
- `npm run check:next`: Next の型、lint、runtime lease、類似度 model、artifact metadata、fixture build をまとめて確認
...
`userscript.config.mjs` が Classic metadata の正本です。現在の公開物と Tampermonkey への Classic 投入物は `dist/revenue-assistant-userscript.user.js` を正とします。`userscript.next.config.mjs` は未公開 Next candidate の metadata 正本であり、生成物は `.tmp/vite-next-candidate/revenue-assistant-next.candidate.user.js` に限ります。`.tmp` の candidate を公開物または自動更新の正本として扱いません。
...
現在の workflow は `src/**`、`scripts/**`、package / build config などの変更を Classic として build / publish します。現在の先行8コミットには Classic entry の source 変更も含まれるため、Next 用の別 workflow / Pages を足すだけでは Classic の再公開を防げません。Classic workflow の source input を公開 baseline へ固定するか、Next-only の source と履歴を Classic workflow から分離し、Classic 公開 artifact hash が byte-for-byte 不変であることを検証するまで、current branch を `main` へ push しません。Next の公開、Tampermonkey install、Classic からの切替は、それぞれ明示承認を受ける別 gate です。
