...
- 現在のNext公開版`0.2.0.32`は、Top background 50ms / concurrency 20、Analyze foreground 35ms / concurrency 20、Top 800 / 200、scope別coalesced refreshを維持しつつ、Analyzeのroom差分更新をdirty scope単位へ狭めた。`0.2.0.31`のホテル関西liveでは最大同時20、全件GET / HTTP 200、write 0でも最大3,927msのLong Taskを再現し、差分後にhotel + 全roomのcurve / DOMを作り直していたことが主因だった。room更新時は全体curveと無関係roomのDOMを再利用し、全体summary / rank overviewとdirty roomだけを更新する。更新後の同施設liveでは自然な差分64件を持つ2 roomの根拠表示が1,488〜1,658msとなり、Long Taskは各0、軽量DOM probeは28 / 28成功した。取得対象、due / dedupe、endpoint、storage / retention、retry / stop、Revenue Assistant write 0は変更していない。初期6 room summary 7,258msの単発観測と20 sample未満のSLOは引き続きprovisionalとする。
