...
- 現在のNext公開版`0.2.0.31`は、Top backgroundの800 / 200とAnalyze foregroundを分離し、Topの可視hotel current全日優先とAnalyzeのscope別coalesced refreshを維持した上で、有限foregroundを35ms / concurrency 20とする。`0.2.0.30`のホテル関西liveでは35ms paceとGET / HTTP 200 / write 0を確認した一方、最大同時30の連続取得中に数秒の操作応答悪化を再現したため、開始速度を落とさず同時滞留だけ20へ戻した。Top backgroundの50ms / concurrency 20、取得対象、due / dedupe、endpoint、storage / retention、retry / stop、Revenue Assistant write 0は変更していない。保存済みsourceは先に表示し、cold scopeは`比較準備中`のまま完了scopeから描画する。`0.2.0.31`のTampermonkey更新後live再確認と、自然revalidate / 20 sample SLOの継続観測を残す。
...
現在の公開Nextはversion `0.2.0.31`、source `d6d9ef203412be3d274dae6b47074320cbdd946d`、manual workflow run `31774630650`です。userscriptは367,856 bytes、SHA-256 `3CD3171DFBC41CBB2D01EAAA71933F6EB49716A555CBCFDE87F2E6F6B6E5B797`、source mapは1,458,811 bytes、SHA-256 `752EDC2B7BCCE95C31BB44EB4948F956D9C337D998CC922275F517859EC7C12B`でrelease manifestと一致し、Classic userscript / source mapは固定baselineと同一byte列です。
