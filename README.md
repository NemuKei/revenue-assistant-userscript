...
- Next公開版`0.2.0.20`は、`0.2.0.19`の通常Chromeで確認した販売設定card内の試験section、方向判定、F5時の`比較準備中`、route cleanup / 再生成を維持しつつ、販売設定と標準booking curveが同じrank status / rank sequencesを別々に取得できる経路を修正した。両runtimeの同一key読取をpage-memoryで共有し、一方のsurface離脱では他方の取得を維持し、route reset後は成功 / 失敗cacheを破棄してfresh readへ戻す。source、focused / full checks、desktop / 390px fixture、main同期、manual publication、公開artifact / Classic baseline照合は完了した。Tampermonkey更新後の通常Chromeで、販売設定から標準booking curveへのtab切替を含むlive総request budget / write 0確認を残す。installed version / 有効状態はmutable stateなので次のlive作業前にもfresh確認し、Classic再公開は引き続き別gateとする。
