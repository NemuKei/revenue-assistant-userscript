...
- 現在のNext公開版`0.2.0.27`は、`0.2.0.26`の5段階queueと未選択room reference遅延を維持しつつ、booking curve開始間隔を100msから50ms、concurrency上限を30から20へ変更した可逆な第一段階candidateである。bootstrap 800 / daily 200、interactive reserve 32、取得対象、due判定、保存、停止条件は変えていない。source `8d9ff5469920005565ced6e9e359160f2627f87b`、manual workflow run `31679952896`のbuild / deploy / verify、公開artifact / Classic baseline照合、Tampermonkey更新、Top / Analyze各3回の通常Chrome表示確認まで完了した。別の自施設の自然なcold windowではbackground 768件を約133秒で完了し、最大同時20、外部timestamp最短51ms、candidate error / 401 / 403 / 429 / Revenue Assistant write 0だったため、50ms profileの負荷安全sub-gateは通過した。一方、同じsessionでAnalyzeがreserve 32件を使い切った後は選択room currentが23msでreadyでもreference根拠が20秒以内にsettleせず、shared budgetによるSLO miss候補を1回観測した。50ms candidateは継続するが、35ms化、critical token、半年取得へはまだ進まない。
