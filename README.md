...
- 現在のNext公開版`0.2.0.38`は、Top background 50ms / concurrency 20、Analyze foreground 35ms / concurrency 20、Top 800 / 200、scope別coalesced refreshを維持しつつ、Analyzeのroom差分更新をdirty scope単位へ狭め、保存済みbooking curveの重複model構築を削減した。booking curve、90日価格推移、競合価格では、独自Tooltipと重なるbrowser native Tooltipを発生させるSVG `title`を持たず、読み上げ名を`aria-label`、説明を`desc` + `aria-describedby`で維持する。取得対象、due / dedupe、endpoint、storage / retention、retry / stop、Revenue Assistant write 0は変更していない。20 sample未満のSLOは引き続きprovisionalとする。
